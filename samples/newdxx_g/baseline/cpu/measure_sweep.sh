...
# (single-rank kernel, executes no MPI calls, no inter-rank communication)
# loading the same dump set on its own disjoint core set -- the sweep
# measures how per-kernel latency behaves under node partitioning /
# co-location.  Every rank verifies against the dumped ground truth before
# its time counts (VERIFY gate).  Mirrors vexx_bp_k/baseline/cpu/measure_sweep.sh.
#
# WARMUP/REPS live HERE, not in the driver: each invocation below is a fresh
# lane launch running verify_newdxx with its default reps=0 (exactly one
# verified kernel call per process), so every rep is a like-for-like
# cold-start measurement, uniform across lanes and reps; the first WARMUP
# invocations are still PASS-gated but their rows are discarded.  (For
# steady-state single-process numbers use the driver's own [reps] [warmup]
# args instead.)  newdxx_g is ms-scale: a full default sweep costs minutes,
# dominated by process launch + deck loading, not kernel time.
#
# SCALING NOTE: newdxx_g's OMP parallel dimension is ATOMS (nat=5 on
# BaTiO3_nat005, 2 on BaO_nat002), so lanes with nt > nat buy nothing --
# expect the sweep to show a hard per-kernel latency floor past nt=nat
# (see README.md and ../../BENCH_RESULTS.md).
#
...
#           SET=0 DUMP=../../data/$MAT CSV=measure_sweep.csv
...
if [ "${1:-}" = "--list" ] || [ "${1:-}" = "-l" ]; then
  echo "lanes for TOTAL=$TOTAL MIN_CORES=$MIN_CORES NPS=\"$NPS\" NTS=\"$NTS\" (deck $DUMP, set $SET):"
  count=0
  printf "%-6s %-9s %s\n" "np" "threads" "cores"
  for np in $NPS; do
    for nt in $NTS; do
...
[ -x ./verify_newdxx ] || { echo "run ./build.sh first" >&2; exit 1; }
[ -f "$DUMP/ndx_${SET}_meta.txt" ] || { echo "no dump set $SET in $DUMP" >&2; exit 1; }
...
for np in $NPS; do
  for nt in $NTS; do
    ...
    [ $cores -gt $TOTAL ] && continue
    echo
    echo "==== np=$np x threads=$nt  ($cores cores)  set $SET  warmup=$WARMUP reps=$REPS ===="
    ...
    for inv in $(seq 1 $(( WARMUP + REPS ))); do
      out=$(OMP_NUM_THREADS=$nt "${launch[@]}" ./verify_newdxx "$DUMP" "$SET" 2>&1)
      rc=$?
    ...
    if [ "$lane_ok" = "1" ]; then
      echo "$lane_times" | tr " " "\n" | awk 'NF{v=$1; s+=v; if(mn==""||v<mn)mn=v; if(v>mx)mx=v; k++}
         END{if (k==0) {print "lane summary: NO SAMPLES"; exit 1}
             printf "lane summary: samples=%d (ranks x reps)  kernel_s min=%.6f max=%.6f mean=%.6f\n", k, mn, mx, s/k}'
    fi
  done
done
echo
