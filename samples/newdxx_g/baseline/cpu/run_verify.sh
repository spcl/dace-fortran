#!/bin/bash
# Correctness gate: verify the standalone newdxx_g kernel against every
...
DUMP=${1:-../../data/$MAT}
THREADS=${2:-"32"}
if ! compgen -G "$DUMP/ndx_*_meta.txt" > /dev/null 2>&1; then
  echo "ERROR: no deck at '$DUMP' (no ndx_*_meta.txt)." >&2
...
for nt in $THREADS; do
  ...
  for set in 0 1 2 3; do
    [ -f "$DUMP/ndx_${set}_meta.txt" ] || { echo "set $set: no dump, skip"; continue; }
    echo "---- set $set ----"
    if ! ./verify_newdxx "$DUMP" "$set"; then
