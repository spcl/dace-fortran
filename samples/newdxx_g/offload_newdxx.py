...
def _configure_library_nodes(sdfg: dace.SDFG):
    ...
    a kernel, and cuBLAS has no callable device-side API either.  A host-launched
    BLAS node gets GPU_Device *and* cuBLAS, the counterpart of the OpenBLAS pick
    ``optimize_sdfg_newdxx_g.assign_blas_implementations`` makes on the CPU lane."""
        ...
        for state in g.all_states():
            ...
            for node in state.nodes():
                ...
                if isinstance(node, Reduce):
                    ...
                    wanted = "pure" if device_side else "cuBLAS"
                    if wanted in type(node).implementations:
                        node.implementation = wanted
