...
SDFG FILE (``newdxx_g_original.sdfg``) of this sample, emit bindings +
...
NOTE: this sample's .dacecache/newdxx_g is WIPED for fresh codegen each run; it
is shared with the run_regex_default/build_opt_lib flows (all regenerable;
the *.handpatched.bak backups are never touched).

    cd /workspace/dace-fortran/samples/newdxx_g && PYTHONHASHSEED=0 python3 run_optimize_original.py
"""
import os
import re
import shutil
...
_GUARD_NEWDXX = """    ! ----- STRICT SCOPE GUARD (injected by run_optimize_original.py) -----
...
KERNELS = {
    "newdxx_g": {"entry": "us_exx::newdxx_g", "guard": _GUARD_NEWDXX},
}
