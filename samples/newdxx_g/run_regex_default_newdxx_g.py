#!/usr/bin/env python3
"""Stage-2-onward walk for ``us_exx::newdxx_g`` (clone of
samples/vexx_bp_k/run_regex_default_vexx_bp_k.py): ingest the READY single TU
``outputs/newdxx_g.F90`` (produced by f2dace-qe-source/
run_regex_default_newdxx_g.py's stage 1 -- no merge stage here; edit that
file to change what flang sees) and produce

  outputs/newdxx_g.hlfir             flang -fc1 -emit-hlfir
  outputs/newdxx_g_normalized.mlir   after DEFAULT_PIPELINE
  outputs/newdxx_g.sdfg              built SDFG
  outputs/lib/                        every compiled module (.mod) + mpi stub,
                                      then the bindings + .so chain:
                                      newdxx_g_bindings.f90, libnewdxx_g.so,
                                      libdace_kernel_newdxx_g.so (+dacestub)

newdxx_g is purely rank-local (its only stubbed callees are start_clock/
stop_clock/errore -- no MPI collectives are dropped), but it still needs a
strict scope guard: hlfir-strip-character-runtime folds every CHARACTER
compare to "equal", so the ``flag`` dispatch collapses to its FIRST arm and
the SDFG hard-bakes the flag='c' complex (k-point) path -- the gamma-trick
'r'/'i' paths would silently compute garbage.  Stage 6 injects the guard and
performs the renamed-kernel relink that keeps the binding .so (same basename
as the dace kernel) from satisfying its own kernel NEEDED entry.

    cd /workspace/dace-fortran/samples/newdxx_g && python3 run_regex_default_newdxx_g.py
"""
import subprocess
import sys
import time
...
OUT = HERE / "outputs"
LIB = OUT / "lib"
SRC_TU = OUT / "newdxx_g.F90"
NAME = "newdxx_g"
BIND_NAME = "newdxx_g"
ENTRY = "us_exx::newdxx_g"
...
#: strict scope guard injected into the generated wrapper (see docstring)
_GUARD_ANCHOR = "    ! ----- Symbol population (input-derived; before allocates) -----"
_GUARD = """    ! ----- STRICT SCOPE GUARD (injected by run_regex_default_newdxx_g.py) -----
    ...
    if (gamma_only__mod) then
      write (*, '(a)') 'FATAL(newdxx_g_dace): gamma_only = .true. -- only the &
        &k-point (flag=''c'') path is baked into this SDFG build; aborting.'
      error stop 8
    end if
    if (.not. present(becphi_c)) then
      write (*, '(a)') 'FATAL(newdxx_g_dace): becphi_c missing -- the &
        &flag=''c'' path needs the complex bec array; aborting.'
      error stop 8
    end if
    if (present(becphi_r)) then
      write (*, '(a)') 'FATAL(newdxx_g_dace): becphi_r passed -- the &
        &gamma (flag=''r''/''i'') path is not in this SDFG build; aborting.'
      error stop 8
    end if
"""
...
def main() -> int:
    ...
    # ---------------------------------------------------------------- stage 6
    stage(6, "inject strict flag='c' guard + relink")
    t0 = time.time()
