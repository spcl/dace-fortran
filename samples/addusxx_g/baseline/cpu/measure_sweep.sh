...
# lane launch running verify_addusxx with its default reps=0 (exactly one
...
# args instead.)  addusxx_g is ms-scale: a full default sweep costs minutes,
...
[ -x ./verify_addusxx ] || { echo "run ./build.sh first" >&2; exit 1; }
[ -f "$DUMP/adx_${SET}_meta.txt" ] || { echo "no dump set $SET in $DUMP" >&2; exit 1; }
...
for np in $NPS; do
  for nt in $NTS; do
    ...
    for inv in $(seq 1 $(( WARMUP + REPS ))); do
      out=$(OMP_NUM_THREADS=$nt "${launch[@]}" ./verify_addusxx "$DUMP" "$SET" 2>&1)
      rc=$?
