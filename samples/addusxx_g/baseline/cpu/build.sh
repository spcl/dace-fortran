#!/bin/bash
# Build the standalone CPU (OpenMP) baseline for QE's us_exx::addusxx_g.
#
# Everything is local to this directory: ONE translation unit
# (addusxx_g_baseline_cpu_omp.f90 = the OMP-preserving merged us_exx closure
# from the QE 7.6 export -- PW/src/us_exx.f90 verified identical to the
# pristine qe-omp2 tree -- + QE utility stubs; regenerate with
# f2dace-qe-source/run_regex_omp_addusxx_g.py) and the verify/bench driver.
#
...
# vexx_bp_k, addusxx_g is a pure G-space kernel with no FFT calls.
#
...
TU=addusxx_g_baseline_cpu_omp.f90
DRIVER=verify_addusxx.f90
BIN=verify_addusxx
