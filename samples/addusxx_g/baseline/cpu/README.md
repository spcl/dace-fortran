# addusxx_g CPU/OpenMP baseline (standalone)

Self-contained Fortran baseline for Quantum ESPRESSO's US-augmentation
kernel `us_exx::addusxx_g` (the `flag='c'` k-point arm invoked from
`exx_bp::vexx_bp_k`; CPU, OpenMP) — compiles and runs from this directory
alone, no QE build required.  The kernel is byte-faithful QE 7.6:
...
FFTW dependency — addusxx_g is a pure G-space kernel.
...
Verifies every present set of the deck against the dumped pw.x ground truth
(bound 1e-11 rel).  Observed: BITWISE-IDENTICAL (`max|diff| = 0.0`) on both
decks at 1/8/32 threads — the kernel's accumulation order is thread-count
independent (static-schedule block ownership).  Expected tail:
`ALL <n> VERIFICATIONS PASSED`.

### 4. Benchmark — driver bench mode

    OMP_NUM_THREADS=32 ./verify_addusxx ../../data/BaTiO3_nat005 0 10 2

Args: `<dumpdir> <set> [reps] [warmup]`; `reps=0` (default) = one verified
call.  With reps>0 every timed call is verified (rhoc reset to the dumped
...
set 0 ~0.19 s at 1T, ~5.5 ms at 32T (parallelism is over ~73 G-blocks —
scales well).
...
Lanes are ms-scale — a full default sweep costs minutes.

## Contents

| file | role |
|---|---|
| `addusxx_g_baseline_cpu_omp.f90` | single TU: the OMP-preserving merged `us_exx` closure (QE 7.6 export, `!$omp` intact) + QE utility stubs (no-op clocks, loud `errore`).  Regenerate: `f2dace-qe-source/run_regex_omp_addusxx_g.py` |
| `verify_addusxx.f90` | driver: loads a dump set into the TU's module state, calls the REAL `addusxx_g`, verifies + times; usage above |
...
| `../../data/<MAT>/` | per-material decks: `adxndx_static_*` (shared with newdxx_g) + `adx_<set>_*` + `MANIFEST.md5` + `PROVENANCE.txt` |
...
convention); a bare `./verify_addusxx` normally works without them at these
problem sizes.
...
* Ground truth `adx_<set>_rhoc_out.bin` = rhoc after the in-QE call, with
  the kernels themselves untouched (see each deck's `PROVENANCE.txt`).
* `nij_type` holds CUMULATIVE species offsets: qgm column =
  `nij_type(nt) + ijtoh(ih,jh,nt)`.
* The TU also carries `newdxx_g` (same module) plus the qvan_init closure
...
`f2dace-qe-source/run_regex_omp_addusxx_g.py` over
