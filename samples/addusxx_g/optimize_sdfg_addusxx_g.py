#!/usr/bin/env python3
"""Run the dace_fortran parallelization pipeline on the built addusxx_g SDFG
...
2. GATE COPY-IN REPAIR (bug 3 of dace-fortran-fixes-needed-6c99810.md): the shipped
   ``outputs/addusxx_g.sdfg`` was BUILT without the monkeypatch above (the builder
...
Output: ``outputs/addusxx_g_opt.sdfg`` (binding signature refrozen, same array ABI).

    cd /workspace/dace-fortran/samples/addusxx_g && \
        PYTHONHASHSEED=0 python3 optimize_sdfg_addusxx_g.py
"""
import argparse
import sys
import time
