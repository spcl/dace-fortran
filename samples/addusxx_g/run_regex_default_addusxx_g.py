#!/usr/bin/env python3
"""Stage-2-onward walk for ``us_exx::addusxx_g`` (clone of
samples/vexx_bp_k/run_regex_default_vexx_bp_k.py): ingest the READY single TU
``outputs/addusxx_g.F90`` (produced by f2dace-qe-source/
run_regex_default_addusxx_g.py's stage 1 -- no merge stage here; edit that
file to change what flang sees) and produce

  outputs/addusxx_g.hlfir             flang -fc1 -emit-hlfir
  outputs/addusxx_g_normalized.mlir   after DEFAULT_PIPELINE
  outputs/addusxx_g.sdfg              built SDFG
  outputs/lib/                        every compiled module (.mod) + mpi stub,
                                      then the bindings + .so chain:
                                      addusxx_g_bindings.f90, libaddusxx_g.so,
                                      libdace_kernel_addusxx_g.so (+dacestub)

addusxx_g is purely rank-local (its only stubbed callees are start_clock/
...
as the dace kernel) from satisfying its own kernel NEEDED entry.
    ...
    cd /workspace/dace-fortran/samples/addusxx_g && python3 run_regex_default_addusxx_g.py
"""
import subprocess
import sys
import time
...
OUT = HERE / "outputs"
LIB = OUT / "lib"
SRC_TU = OUT / "addusxx_g.F90"
NAME = "addusxx_g"
BIND_NAME = "addusxx_g"
ENTRY = "us_exx::addusxx_g"
...
_GUARD = """    ! ----- STRICT SCOPE GUARD (injected by run_regex_default_addusxx_g.py) -----
    ...
    if (present(becphi_r) .or. present(becpsi_r)) then
      write (*, '(a)') 'FATAL(addusxx_g_dace): real bec arrays passed -- the &
