! Numerical verification of the addusxx_g SDFG binding (outputs/lib/
! libaddusxx_g.so) against QE-dumped ground truth (data/BaO_nat002).
!
! Per set: load the marshalled module state (static tables + per-set
! qgm/nij_type), build the dfftt descriptor, call addusxx_g_dace on the
! dumped rhoc_in, and gate against rhoc_out:  max|out - ref| <= 1e-11 *
! max(1, max|ref|).  Scope: flag='c' arm (hard-baked in the SDFG), okvan=T,
! gamma_only=F, single rank.
!
!   usage: verify_addusxx [datadir]     (default data/BaO_nat002; sets 0..1)
program verify_addusxx
  ...
  use addusxx_g_dace_bindings, only: addusxx_g_dace, addusxx_g_dace_finalize
  implicit none
  integer, parameter :: dp = 8
  character(len=512) :: ddir
  type(fft_type_descriptor) :: dfftt
  complex(dp), allocatable :: rhoc(:), ref(:), becphi_c(:), becpsi_c(:)
  ...
  do iset = 0, 1
    ...
    if (.not. allocated(rhoc)) then
      allocate( rhoc(nnr), ref(nnr), becphi_c(nkb), becpsi_c(nkb), bec_r(nkb) )
      bec_r = 0.0_dp   ! dummies: flattened input checks demand them; r/i arms not baked
    end if
    write(f,'(A,A,I0,A)') trim(ddir), '/adx_', iset, '_rhoc_in.bin';  call rd_c(trim(f), rhoc, nnr)
    ...
    !
    call addusxx_g_dace(dfftt, rhoc, xkq, xk, becphi_c=becphi_c, becpsi_c=becpsi_c, &
                        becphi_r=bec_r, becpsi_r=bec_r)
    !
    ! raw SDFG output for offline forensics
    write(f,'(A,A,I0,A)') trim(ddir), '/adx_', iset, '_out_sdfg.bin'
    call wr_c(trim(f), rhoc, nnr)
    maxref  = max(1.0_dp, maxval(abs(ref)))
    maxdiff = maxval(abs(rhoc - ref))
  ...
  call addusxx_g_dace_finalize()
  if (nfail > 0) then
    write(*,'(A,I0,A)') 'addusxx_g: ', nfail, ' set(s) FAILED'
    error stop 1
  end if
  write(*,'(A)') 'addusxx_g: ALL SETS PASSED'
