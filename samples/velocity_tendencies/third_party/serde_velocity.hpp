// VENDORED third-party header -- the struct-aware ASCII serde reader for the ICON
// velocity_tendencies artifact data (deserialize(t_patch*), deserialize(t_nh_diag*),
// read_array<T>, ARRAY_META_DICT, ...).  This is the authoritative parser for the
// .data format; velocity_data.cpp includes it instead of re-implementing a walker.
//
// Source: VelocityTendenciesPipeline/include/serde_velocity_no_nproma.h
//         github.com/ThrudPrimrose/VelocityTendenciesPipeline @ 4adfb050b691
// Vendored (not read from the checkout via an env var) so the cluster job builds
// without that repository present.  Local modifications, kept minimal:
//   * include renamed to the vendored "velocity_structs.hpp";
//   * added <numeric> (std::reduce) -- upstream got it transitively via dace/dace.h;
//   * .hpp extension so the repo clang-format hook (files: .cpp|.h) does not
//     reformat vendored code away from its upstream byte layout.
// Header defines non-inline functions: include from exactly ONE translation unit.
