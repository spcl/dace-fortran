! OpenACC/OpenMP-annotated twin of tests/icon/atmosphere/velocity_advection_inlined_no_loop_exchange_single_tu.f90,
! for the Fortran baseline lanes only (samples/velocity_tendencies/baselines.sh).  GENERATED by
! scripts/annotate_velocity_acc.py -- edit that, not this.  TU variant: noloopexch.
!
    ...
    DO jb = i_startblk, i_endblk
      ...
      DO jk = slev, elev
        DO jv = i_startidx, i_endidx
          rot_vec(jv, jk, jb) = vec_e(vert_edge_idx(jv, jb, 1), jk, vert_edge_blk(jv, jb, 1)) * geofac_rot(jv, 1, &
...
!$OMP DO PRIVATE(jb, jv, jk, i_startidx, i_endidx) SCHEDULE(guided)
    DO jb = i_startblk, i_endblk
      ...
      DO jk = slev, elev
        DO jv = i_startidx, i_endidx
          p_vert_out(jv, jk, jb) = coeff_int(jv, 1, jb) * p_cell_in(vert_cell_idx(jv, jb, 1), jk, vert_cell_blk(jv, &
        ...
        DO jk = 1, nlev
          DO je = i_startidx, i_endidx
            z_v_grad_w(je, jk, jb) = p_diag % vn_ie(je, jk, jb) * p_patch % edges % inv_dual_edge_length(je, jb) * &
              ...
              jb) * (z_w_v(p_patch % edges % vertex_idx(je, jb, 1), jk, p_patch % edges % vertex_blk(je, jb, 1)) - &
              z_w_v(p_patch % edges % vertex_idx(je, jb, 2), jk, p_patch % edges % vertex_blk(je, jb, 2)))
          END DO
        END DO
        !$ACC END PARALLEL
      END DO
...
!$OMP DO PRIVATE(jb, jk, je, i_startidx, i_endidx) SCHEDULE(guided)
      DO jb = i_startblk, i_endblk
        ...
        DO jk = 1, nlev
          DO je = i_startidx, i_endidx
            z_v_grad_w(je, jk, jb) = z_v_grad_w(je, jk, jb) * p_metrics % deepatmo_gradh_ifc(jk) + p_diag % &
    ...
    DO jb = i_startblk, i_endblk
      ...
      DO jk = 1, nlev
        DO jc = i_startidx, i_endidx
          z_ekinh(jc, jk, jb) = p_int % e_bln_c_s(jc, 1, jb) * z_kin_hor_e(p_patch % cells % edge_idx(jc, jb, 1), &
      ...
      DO jk = 2, nlev
        ...
        END DO
      END DO
      !$ACC LOOP GANG(STATIC: 1) VECTOR TILE(4, 32)
      DO jk = 2, nlev
        DO jc = i_startidx_2, i_endidx_2
          p_diag % ddt_w_adv_pc(jc, jk, jb, ntnd) = p_diag % ddt_w_adv_pc(jc, jk, jb, ntnd) + p_int % e_bln_c_s(jc, &
            1, jb) * z_v_grad_w(p_patch % cells % edge_idx(jc, jb, 1), jk, p_patch % cells % edge_blk(jc, jb, 1)) + &
            p_int % e_bln_c_s(jc, 2, jb) * z_v_grad_w(p_patch % cells % edge_idx(jc, jb, 2), jk, p_patch % cells % &
            edge_blk(jc, jb, 2)) + p_int % e_bln_c_s(jc, 3, jb) * z_v_grad_w(p_patch % cells % edge_idx(jc, jb, 3), &
            jk, p_patch % cells % edge_blk(jc, jb, 3))
        END DO
      END DO
      !$ACC END PARALLEL
      IF (lextra_diffu) THEN
        ...
        DO jk = 1, nlev
          DO je = i_startidx, i_endidx
            p_diag % ddt_vn_apc_pc(je, jk, jb, ntnd) = - (z_kin_hor_e(je, jk, jb) * (p_metrics % coeff_gradekin(je, &
              1, jb) - p_metrics % coeff_gradekin(je, 2, jb)) + p_metrics % coeff_gradekin(je, 2, jb) * &
              z_ekinh(p_patch % edges % cell_idx(je, jb, 2), jk, p_patch % edges % cell_blk(je, jb, 2)) - p_metrics &
              % coeff_gradekin(je, 1, jb) * z_ekinh(p_patch % edges % cell_idx(je, jb, 1), jk, p_patch % edges % &
              cell_blk(je, jb, 1)) + p_diag % vt(je, jk, jb) * (p_patch % edges % f_e(je, jb) + 0.5D0 * &
              (zeta(p_patch % edges % vertex_idx(je, jb, 1), jk, p_patch % edges % vertex_blk(je, jb, 1)) + &
              zeta(p_patch % edges % vertex_idx(je, jb, 2), jk, p_patch % edges % vertex_blk(je, jb, 2)))) + (p_int &
              % c_lin_e(je, 1, jb) * z_w_con_c_full(p_patch % edges % cell_idx(je, jb, 1), jk, p_patch % edges % &
              cell_blk(je, jb, 1)) + p_int % c_lin_e(je, 2, jb) * z_w_con_c_full(p_patch % edges % cell_idx(je, jb, &
              2), jk, p_patch % edges % cell_blk(je, jb, 2))) * (p_diag % vn_ie(je, jk, jb) - p_diag % vn_ie(je, jk &
              + 1, jb)) / p_metrics % ddqz_z_full_e(je, jk, jb))
          END DO
        END DO
        !$ACC END PARALLEL
        ...
        DO jk = 1, nlev
          DO je = i_startidx, i_endidx
            p_diag % ddt_vn_apc_pc(je, jk, jb, ntnd) = - ((z_kin_hor_e(je, jk, jb) * (p_metrics % coeff_gradekin(je, &
              ...
              % f_e(je, jb) + 0.5D0 * (zeta(p_patch % edges % vertex_idx(je, jb, 1), jk, p_patch % edges % &
              vertex_blk(je, jb, 1)) + zeta(p_patch % edges % vertex_idx(je, jb, 2), jk, p_patch % edges % &
              ...
              IF (ABS(w_con_e) > cfl_w_limit * p_metrics % ddqz_z_full_e(je, jk, jb)) THEN
                ...
                p_diag % ddt_vn_apc_pc(je, jk, jb, ntnd) = p_diag % ddt_vn_apc_pc(je, jk, jb, ntnd) + difcoef * &
                  ...
                  * p_prog % vn(p_patch % edges % quad_idx(je, jb, 3), jk, p_patch % edges % quad_blk(je, jb, 3)) + &
                  ...
                  inv_primal_edge_length(je, jb) * (zeta(p_patch % edges % vertex_idx(je, jb, 2), jk, p_patch % &
                  edges % vertex_blk(je, jb, 2)) - zeta(p_patch % edges % vertex_idx(je, jb, 1), jk, p_patch % edges &
                  % vertex_blk(je, jb, 1))))
              END IF
            END DO
          END IF
