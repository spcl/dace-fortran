...
namespace flags {
namespace detail {
using argument_map = std::unordered_map<std::string_view, std::vector<std::optional<std::string_view>>>;
...
struct parser {
  ...
  const std::vector<std::string_view> &skipped_tokens() const { return skipped_tokens_; }
...
inline std::vector<std::optional<std::string_view>> get_values(const argument_map &options,
...
// nullopt.
template <class T> std::optional<T> get(const argument_map &options, const std::string_view &option) {
...
template <> inline std::optional<std::string_view> get(const argument_map &options, const std::string_view &option) {
  return get_value(options, option);
}

template <> inline std::optional<std::string> get(const argument_map &options, const std::string_view &option) {
...
template <> inline std::optional<bool> get(const argument_map &options, const std::string_view &option) {
...
template <class T>
std::vector<std::optional<T>> get_multiple(const argument_map &options, const std::string_view &option) {
...
// Since the values are already stored as strings, there's no need to use `>>`.
template <>
inline std::vector<std::optional<std::string_view>> get_multiple(const argument_map &options,
                                                                 const std::string_view &option) {
  return get_values(options, option);
}

template <>
inline std::vector<std::optional<std::string>> get_multiple(const argument_map &options,
                                                            const std::string_view &option) {
  const auto views = get_values(options, option);
...
// for being present.
template <>
inline std::vector<std::optional<bool>> get_multiple(const argument_map &options, const std::string_view &option) {
  const auto views = get_values(options, option);
  ...
  for (const auto view : views) {
    ...
    values.push_back(
        std::none_of(falsities.begin(), falsities.end(), [&view](auto falsity) { return view == falsity; }));
  }
  return values;
}
...
std::optional<T> get(const std::vector<std::string_view> &positional_arguments, size_t positional_index) {
  if (positional_index < positional_arguments.size()) {
    if (T value; std::istringstream(std::string(positional_arguments[positional_index])) >> value)
      return value;
  }
  return std::nullopt;
}
...
// Since the values are already stored as strings, there's no need to use `>>`.
template <>
inline std::optional<std::string_view> get(const std::vector<std::string_view> &positional_arguments,
...
template <>
inline std::optional<std::string> get(const std::vector<std::string_view> &positional_arguments,
                                      size_t positional_index) {
  if (positional_index < positional_arguments.size()) {
...
struct args {
  ...
  template <class T> std::optional<T> get(const std::string_view &option) const {
    return detail::get<T>(parser_.options(), option);
  }

  template <class T> T get(const std::string_view &option, T &&default_value) const {
    return get<T>(option).value_or(default_value);
  }

  template <class T> std::vector<std::optional<T>> get_multiple(const std::string_view &option) const {
    return detail::get_multiple<T>(parser_.options(), option);
  }

  template <class T> std::vector<T> get_multiple(const std::string_view &option, T &&default_value) const {
  ...
  const std::vector<std::string_view> &skipped() const { return parser_.skipped_tokens(); }
