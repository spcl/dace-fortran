...
HEADER = f"""\
! OpenACC/OpenMP-annotated twin of tests/icon/atmosphere/{SRC.name},
! for the Fortran baseline lanes only (samples/velocity_tendencies/baselines.sh).  GENERATED by
! scripts/annotate_velocity_acc.py -- edit that, not this.  TU variant: {VARIANT}.
!
...
# The no-loop-exchange TU is the same 701 lines with ten (horizontal, jk) nests transposed and the
# matching transients re-laid-out, so every anchor keeps its line number and only the loop header
# text moves.  TILE factors move with it: the port maps directives BY LOOP VARIABLE (see HEADER),
# so a factor that sat on the horizontal loop in the loop-exchange nest stays on the horizontal
# loop here, which is the same tiling with the pair written in the other order.  Entries are
# lineno -> (expected prefix, replacement directives or None to keep them).
NOLOOPEXCH = {
...
if VARIANT == "noloopexch":
    ...
    for lineno, where, expect, directives in INS:
        if where == "before" and lineno in NOLOOPEXCH:
            new_expect, new_directives = NOLOOPEXCH[lineno]
            expect, directives = new_expect, new_directives or directives
        patched.append((lineno, where, expect, directives))
    unused = sorted(set(NOLOOPEXCH) - {ln for ln, w, _, _ in INS if w == "before"})
    if unused:
        raise SystemExit(f"NOLOOPEXCH overrides match no 'before' anchor: {unused}")
    INS = patched
