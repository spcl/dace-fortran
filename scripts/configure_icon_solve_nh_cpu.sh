...
if test -z "${DACE_LIBS_DIR}"; then
  ...
  # The DaCe binding build emits mo_solve_nh_diff.mod into this directory;
  # the patched mo_solve_nonhydro.f90 USEs it.
  FCFLAGS="${FCFLAGS} -I${DACE_LIBS_DIR}"
fi
...
if test -n "${DACE_LIBS_DIR}"; then
  # Append libsolve_nh.so to the FINAL ICON link rule in icon.mk so it comes
  # AFTER ${link_files} (which provides the ICON module symbols the .so's shim
  # forwards reference).  --no-as-needed keeps it live under Ubuntu's default
  # --as-needed because the only references to solve_nh_dace_icon live in a
  # single object file that ld may process before the lib.
  link_old='$(silent_FCLD)$(FC) -o $@ $(make_FCFLAGS) $(FCFLAGS) $(ICON_FCFLAGS) $(LDFLAGS) $(link_files) $(shell . ./collect.extra-libs) $(LIBS)'
  link_new="${link_old} -L${DACE_LIBS_DIR} -Wl,-rpath,${DACE_LIBS_DIR} -Wl,--no-as-needed -l:libsolve_nh.so"
...
PYEOF
      echo "[configure_icon_solve_nh_cpu] icon.mk link rule patched to append libsolve_nh.so"
    fi
  else
