"""Load lu.F90 -> build SDFG -> save SDFG + the generated Fortran binding."""
...
LU = HERE / "tests/npb/lu/lu.F90"

# 1. Load lu.F90 and generate the SDFG.  Entry = module::procedure
#    (flang mangles MODULE lu / dolu -> _QMluPdolu).
...
# 3. Generate + save the Fortran binding (emit only -- no link, so we
#    don't need the original ``lu.mod``).  Compile the SDFG first so the
#    wrapper gets the real ``__program`` arg order; auto-derive the
#    caller interface + AoS->SoA flatten plan from the SDFG snapshot.
