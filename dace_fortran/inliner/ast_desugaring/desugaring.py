    ...
    for node in walk(ast, (f03.External_Stmt, f03.Type_Declaration_Stmt)):
        ...
        for name_node in ext_subprogram_names:
            ...
            if name_str in not_supported:
                raise NotImplementedError(
                    f"Function/subroutine with name {name_str} referenced in EXTERNAL statement, but not yet supported. Possible causes: (1) name shared by several functions, (2) Subroutine_Body not in Module."
                )
