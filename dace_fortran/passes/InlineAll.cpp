...
struct InlineAllPass : public mlir::PassWrapper<InlineAllPass, mlir::OperationPass<mlir::ModuleOp>> {
  ...
  static unsigned sweep(mlir::ModuleOp module, mlir::SymbolTable& symTab, bool rootsOnly) {
    ...
    for (auto call : calls) {
      ...
      if (callee.getBody().getBlocks().size() > 1) {
        if (std::getenv("HLFIR_INLINE_TRACE")) {
          llvm::errs() << "InlineAll: SKIP multi-block " << callee.getSymName() << " ("
                       << callee.getBody().getBlocks().size() << " blocks)\n";
          llvm::errs().flush();
        }
        continue;
      }
      ...
      if (std::getenv("HLFIR_INLINE_TRACE")) {
        llvm::errs() << "InlineAll: inlining " << callee.getSymName() << " into "
                     << call->getParentOfType<mlir::func::FuncOp>().getSymName() << "\n";
        llvm::errs().flush();
      }
      LLVM_DEBUG(llvm::dbgs() << "InlineAll: inlining " << callee.getSymName() << " into "
                              << call->getParentOfType<mlir::func::FuncOp>().getSymName() << "\n");
      ...
      auto result = mlir::inlineCall(interface, cloneCallback, callIface, callableIface, &callee.getBody(),
      ...
      if (mlir::succeeded(result)) {
        ...
        LLVM_DEBUG(llvm::dbgs() << "InlineAll: FAILED to inline " << callee.getSymName() << "\n");
      }
    }
  ...
  void runOnOperation() override {
    ...
    LLVM_DEBUG(llvm::dbgs() << "InlineAll: total " << totalInlined << " call sites inlined\n");
  }
};

}  // anonymous namespace

std::unique_ptr<mlir::Pass> createInlineAllPass() { return std::make_unique<InlineAllPass>(); }
