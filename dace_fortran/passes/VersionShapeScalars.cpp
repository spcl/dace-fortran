...
struct VersionShapeScalarsPass
  ...
  llvm::StringRef getDescription() const final {
    return "SSA-version a scalar reassigned (in a straight line) AFTER an array "
  ...
  /// untouched (``success``).
  mlir::LogicalResult
  versionOne(hlfir::DeclareOp decl,
    ...
    for (auto res : {decl.getResult(0), decl.getResult(1)})
      for (auto *u : res.getUsers()) {
        if (writeTargetOf(u) == res) stores.push_back(u);
    ...
    // / subscript (a *data-access* use) does not feed an ``fir.allocmem`` and is
    // irrelevant -- this keeps loop iterators and loop bounds (which mint
    ...
    for (auto *st : stores) {
      if (st->getBlock() != blk || enclosingLoop(st)) {
        return decl.emitError()
               ...
               << "' is reassigned inside a loop or conditional branch AFTER an "
  ...
  static void redirectMemref(mlir::OpOperand &operand, hlfir::DeclareOp from,
    ...
    if (v == from.getResult(0)) operand.set(to.getResult(0));
    else if (v == from.getResult(1)) operand.set(to.getResult(1));
  }
  ...
  static hlfir::DeclareOp makeVersion(mlir::OpBuilder &builder,
    ...
    auto decl = builder.create<hlfir::DeclareOp>(loc, alloca.getResult(),
                                                 newUniq, /*shape=*/mlir::Value{});
    return decl;
  }
};
