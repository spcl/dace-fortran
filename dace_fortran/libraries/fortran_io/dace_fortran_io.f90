...
!
! iso_c_binding wrappers around Fortran external-file I/O for dace.libraries.fortran_io:
! the library nodes lower a READ/WRITE statement to a C++ tasklet calling these
! bind(c) entry points, so the real Fortran runtime performs the transfer. Each
! open returns a fresh newunit, so concurrent I/O nodes never clash on a unit.
module dace_fortran_io
  use, intrinsic :: iso_c_binding
  implicit none

  ! Namelist slot pool (see below): dace_nml_open claims a slot and stores the
  ! opened group's normalised text; dace_nml_get_* reads from it; dace_nml_close
  ! releases it. Caps concurrently-open groups and each group's text length.
...
contains
  ...
  !
  ! A fixed NAMELIST read needs its group declared at compile time, so no
  ! single shipped routine can serve an arbitrary caller's group. These parse
  ! the standardized ``&group name=value /`` text directly instead: the
  ! group's body is lowercased and comma-normalised once at open, then each
  ! typed getter locates its member by name and reads its value(s).
