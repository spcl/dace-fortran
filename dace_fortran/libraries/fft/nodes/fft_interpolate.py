...
(:file:`FFTXlib/src/fft_interfaces.f90`): FFT to G-space, zero-pad /
...
class FFTInterpolatePure(xf.ExpandTransformation):
    ...
    def expansion(node: 'FFTInterpolate', parent_state: SDFGState, parent_sdfg: SDFG) -> SDFG:
        ...
        # 3. FFT on the input grid: rank-1 via the pure DFT lib node, else an
        # inline FFTW3 tasklet (its _inp/_out connectors would otherwise clash
        # with the outer SDFG's same-named FFTInterpolate connectors).
        if rank == 1:
        ...
        # 4. Symmetric-split spectrum copy: one copy tasklet per 'low'/'high'
        ...
        # 5. IFFT on the output grid (same rank dispatch as the forward FFT);
        # un-normalised, the 1/Nin scaling is applied in the finalize state.
