...
CONTAINS
  ...
  SUBROUTINE YOMCST_LOAD_PARAMETERS()
    ! SC2026: Temp JPRB variables for LOAD_SCALAR, then copy to JPRL
    REAL(KIND=JPRB) :: ZRLVTT, ZRLSTT, ZRLMLT
    CALL LOAD_SCALAR('RG', RG)
    CALL LOAD_SCALAR('RD', RD)
    CALL LOAD_SCALAR('RCPD', RCPD)
    CALL LOAD_SCALAR('RETV', RETV)
    CALL LOAD_SCALAR('RLVTT', ZRLVTT)
    RLVTT = ZRLVTT
    CALL LOAD_SCALAR('RLSTT', ZRLSTT)
    RLSTT = ZRLSTT
    CALL LOAD_SCALAR('RLMLT', ZRLMLT)
    RLMLT = ZRLMLT
