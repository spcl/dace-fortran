"""FV3 GFDL ``warm_rain`` kernel end-to-end: same source runs through the bridge and
f2py on one deterministic column, compared array-by-array. Carved from fv3gfs-fortran's
gdfl_cloud_microphys.F90, self-contained (tuning constants inlined, no derived
types/host init). real(4) kernel widened by the bridge -> loose comparison. Module-scope
...
# Length of the water-saturation lookup tables (tablew/desw); module parameter qs_length in the source.
_QS_LENGTH = 2621

# Arguments warm_rain mutates (intent inout/out), in signature order after the
# scalars -- these are what we compare between the two paths.
