MODULE mo_fortran_tools
  ...
  SUBROUTINE init_zero_3d_dp(init_var, lacc, opt_acc_async_queue)
    ...
    CALL set_acc_host_or_device(lzacc, .FALSE.)
...
MODULE mo_ocean_nml
  ...
  NAMELIST /ocean_dynamics_nml/ i_bc_veloc_lateral, n_zlev, fast_performance_level, nonlinearcoriolis_type
  CONTAINS
END MODULE mo_ocean_nml
MODULE mo_ocean_types
  USE mo_math_types, ONLY: t_cartesian_coordinates
  TYPE :: t_hydro_ocean_diag
    REAL(KIND = 8), POINTER, DIMENSION(:, :, :) :: kin
    TYPE(t_cartesian_coordinates), POINTER, DIMENSION(:, :, :) :: p_vn
    REAL(KIND = 8), POINTER, DIMENSION(:, :, :) :: grad
...
MODULE mo_ocean_velocity_advection
  ...
  SUBROUTINE veloc_adv_horz_mimetic_rot(patch_3d, vn, p_diag, veloc_adv_horz_e, ocean_coefficients, lacc)
    ...
    USE mo_util_dbg_prnt, ONLY: dbg_print_3d_deconiface_73 => dbg_print_3d, dbg_print_3d_deconiface_74 => dbg_print_3d, dbg_print_3d_deconiface_75 => dbg_print_3d
    TYPE(t_patch_3d), POINTER, INTENT(IN) :: patch_3d
    ...
    END DO
    idt_src = 3
    CALL dbg_print_3d_deconiface_73('HorzMimRot: kin energy', p_diag % kin, str_module, 3, patch_2d % cells % owned)
    CALL dbg_print_3d_deconiface_74('HorzMimRot: vorticity', p_diag % vort, str_module, 3, patch_2d % verts % owned)
    CALL dbg_print_3d_deconiface_75('HorzMimRot: grad kin en', p_diag % grad, str_module, 3, patch_2d % edges % owned)
  ...
  SUBROUTINE calculate_only_kineticgrad(patch_3d, vn, p_diag, veloc_adv_horz_e, ocean_coefficients)
    USE mo_model_domain, ONLY: t_patch, t_patch_3d, t_subset_range
    USE mo_ocean_types, ONLY: t_hydro_ocean_diag, t_operator_coeff
    USE mo_fortran_tools, ONLY: init_zero_3d_dp_deconiface_76 => init_zero_3d_dp
    USE mo_grid_subset, ONLY: get_index_range
    USE mo_ocean_math_operators, ONLY: grad_fd_norm_oce_3d_onblock
    USE mo_util_dbg_prnt, ONLY: dbg_print_3d_deconiface_77 => dbg_print_3d, dbg_print_3d_deconiface_78 => dbg_print_3d
    ...
    patch_2d => patch_3d % p_patch_2d(1)
    edges_in_domain => patch_2d % edges % in_domain
    CALL init_zero_3d_dp_deconiface_76(veloc_adv_horz_e, lacc = .FALSE.)
    DO blockno = edges_in_domain % start_block, edges_in_domain % end_block
      CALL get_index_range(patch_2d % edges % in_domain, blockno, start_edge_index, end_edge_index)
    ...
    END DO
    idt_src = 3
    CALL dbg_print_3d_deconiface_77('HorzMimRot: kin energy', p_diag % kin, str_module, 3, patch_2d % cells % owned)
    CALL dbg_print_3d_deconiface_78('HorzMimRot: grad kin en', p_diag % grad, str_module, 3, patch_2d % edges % owned)
  ...
  SUBROUTINE veloc_adv_horz_mimetic_classiccgrid(patch_3d, vn, p_diag, veloc_adv_horz_e, ocean_coefficients)
    ...
    USE mo_sync, ONLY: sync_patch_array_3d_dp_deconiface_79 => sync_patch_array_3d_dp
    USE mo_grid_subset, ONLY: get_index_range
    USE mo_util_dbg_prnt, ONLY: dbg_print_2d_deconiface_82 => dbg_print_2d, dbg_print_3d_deconiface_80 => dbg_print_3d, dbg_print_3d_deconiface_81 => dbg_print_3d, dbg_print_3d_deconiface_83 => dbg_print_3d, dbg_print_3d_deconiface_84 => dbg_print_3d
    TYPE(t_patch_3d), TARGET, INTENT(IN) :: patch_3d
    ...
    CALL sync_patch_array_3d_dp_deconiface_79(3, patch_3d % p_patch_2d(1), p_diag % vort, lacc = .FALSE.)
    DO blockno = edges_in_domain % start_block, edges_in_domain % end_block
      CALL get_index_range(patch_2d % edges % in_domain, blockno, start_edge_index, end_edge_index)
      DO je = start_edge_index, end_edge_index
    ...
    END DO
    idt_src = 3
    CALL dbg_print_3d_deconiface_80('advHorCgrid: kin energy', p_diag % kin, str_module, 3, patch_2d % cells % owned)
    CALL dbg_print_3d_deconiface_81('advHorCgrid: vorticity', p_diag % vort, str_module, 3, patch_2d % verts % owned)
    CALL dbg_print_2d_deconiface_82('advHorCgrid: f_e', patch_2d % edges % f_e, str_module, 3, patch_2d % edges % owned)
    CALL dbg_print_3d_deconiface_83('advHorCgrid: grad kin en', p_diag % grad, str_module, 3, patch_2d % edges % owned)
    CALL dbg_print_3d_deconiface_84('advHorCgrid: veloc_adv_horz_e', veloc_adv_horz_e, str_module, 3, patch_2d % edges % owned)
