...
MODULE mo_ocean_solve_backend
  ...
  SUBROUTINE ocean_solve_backend_dump_matrix(this, id, lprecon, lacc)
    USE mo_fortran_tools, ONLY: set_acc_host_or_device
    USE mo_exception, ONLY: finish
    USE mo_ocean_solve_lhs, ONLY: lhs_dump_matrix_deconproc_58 => lhs_dump_matrix, lhs_dump_matrix_deconproc_59 => lhs_dump_matrix
    ...
    IF (lprecon) THEN
      CALL lhs_dump_matrix_deconproc_58(this % lhs, id, "ocean_matrix_precon_", .TRUE., lacc = lzacc)
    ELSE
      CALL lhs_dump_matrix_deconproc_59(this % lhs, id, "ocean_matrix_lhs_", .FALSE., lacc = lzacc)
    END IF
...
MODULE mo_ocean_solve
  ...
  SUBROUTINE ocean_solve_dump_matrix(this, id, lprecon_in, lacc)
    USE mo_fortran_tools, ONLY: set_acc_host_or_device
    USE mo_ocean_solve_backend, ONLY: ocean_solve_backend_dump_matrix_deconproc_74 => ocean_solve_backend_dump_matrix
    ...
    CALL set_acc_host_or_device(lzacc, lacc)
    lprecon = .FALSE.
    CALL ocean_solve_backend_dump_matrix_deconproc_74(this % act, id, lprecon, lacc = lzacc)
...
MODULE mo_ocean_ab_timestepping_mimetic
  ...
  SUBROUTINE init_free_sfc_ab_mimetic(patch_3d, ocean_state, op_coeffs, solvercoeff_sp, lacc)
    ...
    USE mo_ocean_solve_aux, ONLY: ocean_solve_parm_init_deconproc_81 => ocean_solve_parm_init, t_ocean_solve_parm
    USE mo_fortran_tools, ONLY: set_acc_host_or_device
    USE mo_parallel_config, ONLY: nproma
    ...
    USE mo_exception, ONLY: finish
    USE mo_ocean_solve_lhs_type, ONLY: lhs_surface_height_construct_deconproc_82 => lhs_surface_height_construct
    USE mo_ocean_solve_transfer, ONLY: subset_transfer_construct_deconproc_84 => subset_transfer_construct, trivial_transfer_construct_deconproc_83 => trivial_transfer_construct, trivial_transfer_construct_deconproc_85 => trivial_transfer_construct
    ...
    patch_2d => patch_3d % p_patch_2d(1)
    CALL ocean_solve_parm_init_deconproc_81(par, 60, 1, 800, patch_2d % cells % in_domain % end_block, patch_2d % alloc_cell_blocks, nproma, patch_2d % cells % in_domain % end_index, solver_tolerance, use_absolute_solver_tolerance)
    par_sp % nidx = (- 1)
    sol_type = 21
    ...
    END SELECT
    CALL lhs_surface_height_construct_deconproc_82(free_sfc_solver_lhs, patch_3d, ocean_state % p_diag % thick_e, op_coeffs, solvercoeff_sp, lacc = lzacc)
    SELECT CASE (select_transfer)
    CASE (0)
      CALL trivial_transfer_construct_deconproc_83(free_sfc_solver_trans_triv, 11, patch_2d, lacc = lzacc)
    ...
    CASE DEFAULT
      ...
      CALL subset_transfer_construct_deconproc_84(free_sfc_solver_trans_sub, 11, patch_2d, ABS(select_transfer), trans_mode, lacc = lzacc)
    ...
    IF (l_solver_compare) THEN
      CALL trivial_transfer_construct_deconproc_85(free_sfc_solver_trans_triv, 11, patch_2d, lacc = lzacc)
  ...
  SUBROUTINE solve_free_sfc_ab_mimetic(patch_3d, ocean_state, p_ext_data, p_as, p_oce_sfc, p_phys_param, timestep, op_coeffs, solvercoeff_sp, ret_status, lacc)
    ...
    USE mo_grid_subset, ONLY: get_index_range
    USE mo_ocean_solve, ONLY: ocean_solve_dump_matrix_deconproc_88 => ocean_solve_dump_matrix, ocean_solve_solve_deconproc_86 => ocean_solve_solve, ocean_solve_solve_deconproc_87 => ocean_solve_solve
    USE mo_dbg_nml, ONLY: idbg_mxmn
    ...
    IF (.NOT. l_rigid_lid) THEN
      ...
      CALL ocean_solve_solve_deconproc_86(free_sfc_solver, n_it, n_it_sp, lacc = lzacc)
      ...
      IF (rn <= solver_tolerance) THEN
        ...
        END DO
        IF (l_is_compare_step) THEN
          CALL ocean_solve_solve_deconproc_87(free_sfc_solver_comp, n_it, n_it_sp, lacc = lzacc)
          ...
          CALL message('ocean_solve(' // TRIM(free_sfc_solver_comp % sol_type_name) // '): surface height', TRIM(string))
        END IF
        IF (createsolvermatrix) CALL ocean_solve_dump_matrix_deconproc_88(free_sfc_solver, timestep, lacc = lzacc)
  ...
  FUNCTION invert_mass_matrix(patch_3d, ocean_state, op_coeffs, rhs_e) RESULT(inv_flip_flop_e)
    ...
    USE mo_ocean_types, ONLY: t_hydro_ocean_state, t_operator_coeff
    USE mo_impl_constants, ONLY: max_char_length
    USE mo_ocean_solve_aux, ONLY: ocean_solve_parm_init_deconproc_91 => ocean_solve_parm_init, t_ocean_solve_parm
    USE mo_ocean_solve_lhs_type, ONLY: lhs_primal_flip_flop_construct_deconproc_89 => lhs_primal_flip_flop_construct, lhs_primal_flip_flop_construct_deconproc_92 => lhs_primal_flip_flop_construct
    USE mo_ocean_solve_transfer, ONLY: trivial_transfer_construct_deconproc_90 => trivial_transfer_construct
    USE mo_ocean_nml, ONLY: massmatrix_solver_tolerance, n_zlev, solver_max_iter_per_restart, solver_max_restart_iterations
    USE mo_ocean_solve, ONLY: ocean_solve_construct__t_primal_flip_flop_lhs__t_triv__1f49ae23, ocean_solve_solve_deconproc_93 => ocean_solve_solve
    USE mo_dbg_nml, ONLY: idbg_mxmn
    ...
    IF (.NOT. inv_mm_solver % is_init) THEN
      CALL lhs_primal_flip_flop_construct_deconproc_89(inv_mm_solver_lhs, patch_3d, op_coeffs, (- 999))
      patch_2d => patch_3d % p_patch_2d(1)
      CALL trivial_transfer_construct_deconproc_90(inv_mm_solver_trans, 12, patch_2d)
      CALL ocean_solve_parm_init_deconproc_91(par, 60, solver_max_restart_iterations, solver_max_iter_per_restart, patch_2d % cells % in_domain % end_block, SIZE(rhs_e, 3), SIZE(rhs_e, 1), patch_2d % edges % in_domain % end_index, massmatrix_solver_tolerance, .TRUE.)
      par_sp % nidx = (- 1)
    ...
    END IF
    DO jk = 1, n_zlev
      CALL lhs_primal_flip_flop_construct_deconproc_92(inv_mm_solver_lhs, patch_3d, op_coeffs, jk)
      inv_mm_solver % x_loc_wp(:, :) = 0.0D0
      inv_mm_solver % b_loc_wp => rhs_e(:, jk, :)
      CALL ocean_solve_solve_deconproc_93(inv_mm_solver, n_it, n_it_sp)
