"""Rewrites ICON's mo_solve_nonhydro::solve_nh into either a DaCe-only driver or a differential driver.

* DaCe-only: the original body is kept as ``solve_nh_ref`` (unused) and ``solve_nh`` forwards to the
  SDFG-generated ``solve_nh_dace_icon`` wrapper.
...
#: Driver body that only forwards to the SDFG wrapper (no deep copy / reference run).
_DACE_ONLY_BLOCK = """\
    ! DACE-ONLY driver: forward the call to the SDFG-generated wrapper.  The
    ! original body is preserved as ``solve_nh_ref`` for inspection but is not
    ! invoked here; correctness is checked by comparing model output files.
    INTERFACE
      SUBROUTINE solve_nh_dace_icon(p_nh, p_patch, p_int, prep_adv, &
                                    nnow, nnew, &
                                    l_init, l_recompute, lsave_mflx, &
...
def apply_solve_nh_patch(pristine_source: str, dace_only: bool = False) -> str:
    """Rewrites mo_solve_nonhydro.f90's solve_nh.

    * ``dace_only=False`` (default): differential driver + ``solve_nh_ref``.
    * ``dace_only=True``: forwards to the SDFG wrapper and keeps the original
      body as ``solve_nh_ref`` (unused).

    Raises ValueError if solve_nh isn't found.
    """
...
def write_patched_solve_nh(pristine_path: Path, patched_path: Path, dace_only: bool = False):
    """Read pristine_path, patch, write to patched_path; returns the patched-line count."""
    patched = apply_solve_nh_patch(pristine_path.read_text(), dace_only=dace_only)
