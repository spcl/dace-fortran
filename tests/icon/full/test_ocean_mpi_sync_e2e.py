...
Ocean-side companion to ``test_dycore_mpi_sync_e2e.py`` (atmosphere), same 2-rank
real-MPI shape but with the ocean sync name (``mo_sync::sync_patch_array_3d_dp``) and
an ocean-mimetic owned-block formula.  Runs under ``mpirun --oversubscribe`` at any
even rank count: COMM_WORLD splits into adjacent 2-rank pairs, each running an
independent symmetric halo swap on a per-rank ``(nproma, n_zlev, 2)`` field (block 1
owned, block 2 halo filled via a real ``MPI_Sendrecv``).

``sync_patch_array_3d_dp`` is ICON-shaped but deliberately tiny (self-contained, no
ICON build).  The communicator is a Fortran ``MPI_Fint`` handle carried byte-for-byte
through the C ABI as ``int`` -- no ``MPI_Comm_f2c`` round-trip.  Compiles both as the
gfortran reference and as the DaCe SDFG (sync registered ``keep_external`` so the
...
# Dummy ocean halo-exchange ``sync_patch_array_3d_dp``: ICON-shaped, minimum body that
# exercises a real ``MPI_Sendrecv``.  ``comm`` is a Fortran ``MPI_Fint`` handle (OpenMPI
# = C int) carried byte-for-byte, matching ``mpi4py.MPI.Comm.py2f()``; no
# ``MPI_Comm_f2c``.  MPI constants hard-coded (not ``use mpi``) to avoid the
# flang-vs-OpenMPI .mod dependency at SDFG-bridge build time.
_SYNC_MPI_SRC = r"""
...
# Standalone ocean dycore: per-rank owned-block computation (scaled field + a
# level-weighted ratio), then the sync fills block 2 (halo) from the neighbour's block 1.
_DYCORE_SRC = r"""
...
# (extents-first, scalars-last), so the same ctypes argtypes drive both.
_REF_DRIVER_SRC = r"""
...
def _build_sync_mpi_lib(build_dir: Path) -> Path:
    """Compile mo_ocean_sync_mpi into ``libocean_sync_mpi.so`` via ``mpifort`` (injects
    MPI include/lib paths); carries the bind_c wrapper the SDFG kernel links + dlopens."""
...
def _build_ref_lib(build_dir: Path, sync_so: Path, sync_build_dir: Path) -> Path:
    """Compile the gfortran reference (dycore + driver), linked against the same
    real-MPI sync library the SDFG path uses."""
    build_dir.mkdir(parents=True, exist_ok=True)
...
def test_ocean_dycore_with_real_mpi_sync_2rank(tmp_path: Path):
    """2-rank ocean dycore + real-MPI ``sync_patch_array_3d_dp``: SDFG path vs gfortran
    reference, per-rank bit-exact.  Rank 0 builds every artifact; other ranks wait at a
    barrier then load the same .so files.  ``sync_patch_array_3d_dp`` is registered
    ...
    # tmp_path differs per rank (pytest fixture); pin every rank to rank 0's path so
    # the .so artefacts are shared.
    tmp_path_str = str(tmp_path) if rank == 0 else None
    tmp_path = Path(comm.bcast(tmp_path_str, root=0))

    # build phase: rank 0 only.  build_on_root broadcasts the artefact paths (or a
    # build failure) so other ranks never block waiting for artefacts that won't exist.
    def _build_artifacts():
        ...
        try:
            ...
            # bridge needs mo_ocean_sync_mpi's body as a prelude so the bind_c_shim's
            # USE statement resolves at gfortran link time
    ...
    # barrier so every rank proceeds only once the artefacts exist on the shared fs
    comm.Barrier()

    # pre-load ocean_sync_mpi so the SDFG / reference rpaths resolve to the same instance
    ctypes.CDLL(sync_so_str, mode=ctypes.RTLD_GLOBAL)
    sdfg_lib_obj = ctypes.CDLL(sdfg_so_str)
    ref_lib_obj = ctypes.CDLL(ref_so_str)

    # distinct per-rank seeds: a mis-routed comm surfaces immediately as a data swap.
    # (nproma, n_zlev, 2) -- block 1 owned, block 2 halo.
    ...
    # bit-exact on both owned (block 1) and halo (block 2, via MPI Sendrecv); a
    # mis-routed comm/neighbour diverges immediately, a codegen regression as >1 ULP.
    one_ulp_rtol = 2**-52
    ...
    # halo sanity check: block 2 must equal the neighbour's computed block 1
