    ...
    DO jb = i_startblk, i_endblk
      ...
      DO jk = slev, elev
        DO jv = i_startidx, i_endidx
          rot_vec(jv, jk, jb) = vec_e(vert_edge_idx(jv, jb, 1), jk, vert_edge_blk(jv, jb, 1)) * geofac_rot(jv, 1, jb) + vec_e(vert_edge_idx(jv, jb, 2), jk, vert_edge_blk(jv, jb, 2)) * geofac_rot(jv, 2, jb) + vec_e(vert_edge_idx(jv, jb, 3), jk, vert_edge_blk(jv, jb, 3)) * geofac_rot(jv, 3, jb) + vec_e(vert_edge_idx(jv, jb, 4), jk, vert_edge_blk(jv, jb, 4)) * geofac_rot(jv, 4, jb) + vec_e(vert_edge_idx(jv, jb, 5), jk, vert_edge_blk(jv, jb, 5)) * geofac_rot(jv, 5, jb) + vec_e(vert_edge_idx(jv, jb, 6), jk, vert_edge_blk(jv, jb, 6)) * geofac_rot(jv, 6, jb)
        END DO
      END DO
    END DO
