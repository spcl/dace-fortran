...
MODULE mo_lib_divrot
  IMPLICIT NONE
  CONTAINS
  SUBROUTINE rot_vertex_ri_lib(vec_e, vert_edge_idx, vert_edge_blk, geofac_rot, rot_vec, i_startblk, i_endblk, i_startidx_in, i_endidx_in, slev, elev, nproma, lacc, acc_async)
    ...
    REAL(KIND = 8), INTENT(IN) :: geofac_rot(:, :, :)
    ...
    DO jb = i_startblk, i_endblk
      ...
      DO jv = i_startidx, i_endidx
        DO jk = slev, elev
          rot_vec(jk, jv, jb) = vec_e(vert_edge_idx(jv, jb, 1), jk, vert_edge_blk(jv, jb, 1)) * geofac_rot(jv, 1, jb) + vec_e(vert_edge_idx(jv, jb, 2), jk, vert_edge_blk(jv, jb, 2)) * geofac_rot(jv, 2, jb) + vec_e(vert_edge_idx(jv, jb, 3), jk, vert_edge_blk(jv, jb, 3)) * geofac_rot(jv, 3, jb) + vec_e(vert_edge_idx(jv, jb, 4), jk, vert_edge_blk(jv, jb, 4)) * geofac_rot(jv, 4, jb) + vec_e(vert_edge_idx(jv, jb, 5), jk, vert_edge_blk(jv, jb, 5)) * geofac_rot(jv, 5, jb) + vec_e(vert_edge_idx(jv, jb, 6), jk, vert_edge_blk(jv, jb, 6)) * geofac_rot(jv, 6, jb)
        END DO
      END DO
    END DO
...
MODULE mo_lib_interpolation_scalar
  IMPLICIT NONE
  CONTAINS
  SUBROUTINE cells2verts_scalar_ri_lib(p_cell_in, vert_cell_idx, vert_cell_blk, coeff_int, p_vert_out, i_startblk, i_endblk, i_startidx_in, i_endidx_in, slev, elev, nproma, lacc, acc_async)
    USE mo_fortran_tools, ONLY: set_acc_host_or_device
    USE mo_lib_loopindices, ONLY: get_indices_v_lib
    ...
    DO jb = i_startblk, i_endblk
      ...
      DO jv = i_startidx, i_endidx
        DO jk = slev, elev
          p_vert_out(jk, jv, jb) = coeff_int(jv, 1, jb) * p_cell_in(vert_cell_idx(jv, jb, 1), jk, vert_cell_blk(jv, jb, 1)) + coeff_int(jv, 2, jb) * p_cell_in(vert_cell_idx(jv, jb, 2), jk, vert_cell_blk(jv, jb, 2)) + coeff_int(jv, 3, jb) * p_cell_in(vert_cell_idx(jv, jb, 3), jk, vert_cell_blk(jv, jb, 3)) + coeff_int(jv, 4, jb) * p_cell_in(vert_cell_idx(jv, jb, 4), jk, vert_cell_blk(jv, jb, 4)) + coeff_int(jv, 5, jb) * p_cell_in(vert_cell_idx(jv, jb, 5), jk, vert_cell_blk(jv, jb, 5)) + coeff_int(jv, 6, jb) * p_cell_in(vert_cell_idx(jv, jb, 6), jk, vert_cell_blk(jv, jb, 6))
        END DO
      END DO
    END DO
...
MODULE mo_math_divrot
  IMPLICIT NONE
  CONTAINS
  SUBROUTINE rot_vertex_ri(vec_e, ptr_patch, ptr_int, rot_vec, lacc, opt_slev, opt_elev, opt_rlend, opt_acc_async)
    USE mo_model_domain, ONLY: t_patch
    USE mo_intp_data_strc, ONLY: t_int_state
    USE mo_lib_divrot, ONLY: rot_vertex_ri_lib
    USE mo_parallel_config, ONLY: nproma
    ...
    CALL rot_vertex_ri_lib(vec_e, ptr_patch % verts % edge_idx, ptr_patch % verts % edge_blk, ptr_int % geofac_rot, rot_vec, i_startblk, i_endblk, i_startidx_in, i_endidx_in, slev, elev, nproma, lacc = .TRUE., acc_async = .TRUE.)
...
MODULE mo_icon_interpolation_scalar
  IMPLICIT NONE
  CONTAINS
  SUBROUTINE cells2verts_scalar_ri(p_cell_in, ptr_patch, c_int, p_vert_out, lacc, opt_slev, opt_elev, opt_rlstart, opt_rlend, opt_acc_async)
    USE mo_model_domain, ONLY: t_patch
    ...
    CALL cells2verts_scalar_ri_lib(p_cell_in, ptr_patch % verts % cell_idx, ptr_patch % verts % cell_blk, c_int, p_vert_out, i_startblk, i_endblk, i_startidx_in, i_endidx_in, slev, elev, nproma, lacc = .TRUE., acc_async = .TRUE.)
...
MODULE mo_velocity_advection
  ...
  SUBROUTINE velocity_tendencies(p_prog, p_patch, p_int, p_metrics, p_diag, z_w_concorr_me, z_kin_hor_e, z_vt_ie, ntnd, istep, lvn_only, dtime, dt_linintp_ubc, ldeepatmo)
    ...
    REAL(KIND = 8) :: z_v_grad_w(p_patch % nlev, nproma, p_patch % nblks_e)
    REAL(KIND = 8) :: z_w_v(p_patch % nlevp1, nproma, p_patch % nblks_v)
    REAL(KIND = 8) :: zeta(p_patch % nlev, nproma, p_patch % nblks_v)
    REAL(KIND = 8) :: z_ekinh(p_patch % nlev, nproma, p_patch % nblks_c)
    INTEGER :: nlev, nlevp1
    ...
    END IF
    IF (.NOT. lvn_only) CALL cells2verts_scalar_ri(p_prog % w, p_patch, p_int % cells_aw_verts, z_w_v, lacc = .TRUE., opt_rlend = (- 5), opt_acc_async = .TRUE.)
    CALL rot_vertex_ri(p_prog % vn, p_patch, p_int, zeta, lacc = .TRUE., opt_rlend = (- 5), opt_acc_async = .TRUE.)
    IF (istep == 1) THEN
      rl_start = 5
      rl_end = (- 10)
    ...
    IF (.NOT. lvn_only) THEN
      DO jb = i_startblk, i_endblk
        ...
        DO je = i_startidx, i_endidx
          DO jk = 1, nlev
            z_v_grad_w(jk, je, jb) = p_diag % vn_ie(je, jk, jb) * p_patch % edges % inv_dual_edge_length(je, jb) * (p_prog % w(p_patch % edges % cell_idx(je, jb, 1), jk, p_patch % edges % cell_blk(je, jb, 1)) - p_prog % w(p_patch % edges % cell_idx(je, jb, 2), jk, p_patch % edges % cell_blk(je, jb, 2))) + z_vt_ie(je, jk, jb) * p_patch % edges % inv_primal_edge_length(je, jb) * p_patch % edges % tangent_orientation(je, jb) * (z_w_v(jk, p_patch % edges % vertex_idx(je, jb, 1), p_patch % edges % vertex_blk(je, jb, 1)) - z_w_v(jk, p_patch % edges % vertex_idx(je, jb, 2), p_patch % edges % vertex_blk(je, jb, 2)))
          END DO
        END DO
      END DO
    END IF
    IF (.NOT. lvn_only .AND. ldeepatmo) THEN
      DO jb = i_startblk, i_endblk
        ...
        DO je = i_startidx, i_endidx
          DO jk = 1, nlev
            z_v_grad_w(jk, je, jb) = z_v_grad_w(jk, je, jb) * p_metrics % deepatmo_gradh_ifc(jk) + p_diag % vn_ie(je, jk, jb) * (p_diag % vn_ie(je, jk, jb) * p_metrics % deepatmo_invr_ifc(jk) - p_patch % edges % ft_e(je, jb)) + z_vt_ie(je, jk, jb) * (z_vt_ie(je, jk, jb) * p_metrics % deepatmo_invr_ifc(jk) + p_patch % edges % fn_e(je, jb))
          END DO
        END DO
      END DO
    END IF
    ...
    DO jb = i_startblk, i_endblk
      ...
      DO jc = i_startidx, i_endidx
        DO jk = 1, nlev
          z_ekinh(jk, jc, jb) = p_int % e_bln_c_s(jc, 1, jb) * z_kin_hor_e(p_patch % cells % edge_idx(jc, jb, 1), jk, p_patch % cells % edge_blk(jc, jb, 1)) + p_int % e_bln_c_s(jc, 2, jb) * z_kin_hor_e(p_patch % cells % edge_idx(jc, jb, 2), jk, p_patch % cells % edge_blk(jc, jb, 2)) + p_int % e_bln_c_s(jc, 3, jb) * z_kin_hor_e(p_patch % cells % edge_idx(jc, jb, 3), jk, p_patch % cells % edge_blk(jc, jb, 3))
        END DO
      END DO
      IF (istep == 1) THEN
        DO jc = i_startidx, i_endidx
      ...
      CALL get_indices_c(p_patch, jb, i_startblk_2, i_endblk_2, i_startidx_2, i_endidx_2, rl_start_2, rl_end_2)
      DO jk = 2, nlev
      ...
      DO jc = i_startidx_2, i_endidx_2
        DO jk = 2, nlev
          p_diag % ddt_w_adv_pc(jc, jk, jb, ntnd) = p_diag % ddt_w_adv_pc(jc, jk, jb, ntnd) + p_int % e_bln_c_s(jc, 1, jb) * z_v_grad_w(jk, p_patch % cells % edge_idx(jc, jb, 1), p_patch % cells % edge_blk(jc, jb, 1)) + p_int % e_bln_c_s(jc, 2, jb) * z_v_grad_w(jk, p_patch % cells % edge_idx(jc, jb, 2), p_patch % cells % edge_blk(jc, jb, 2)) + p_int % e_bln_c_s(jc, 3, jb) * z_v_grad_w(jk, p_patch % cells % edge_idx(jc, jb, 3), p_patch % cells % edge_blk(jc, jb, 3))
        END DO
      END DO
      IF (lextra_diffu) THEN
        DO jk = MAX(3, nrdmax_jg - 2), nlev - 3
    ...
    DO jb = i_startblk, i_endblk
      ...
      IF (.NOT. ldeepatmo) THEN
        DO je = i_startidx, i_endidx
          DO jk = 1, nlev
            p_diag % ddt_vn_apc_pc(je, jk, jb, ntnd) = - (z_kin_hor_e(je, jk, jb) * (p_metrics % coeff_gradekin(je, 1, jb) - p_metrics % coeff_gradekin(je, 2, jb)) + p_metrics % coeff_gradekin(je, 2, jb) * z_ekinh(jk, p_patch % edges % cell_idx(je, jb, 2), p_patch % edges % cell_blk(je, jb, 2)) - p_metrics % coeff_gradekin(je, 1, jb) * z_ekinh(jk, p_patch % edges % cell_idx(je, jb, 1), p_patch % edges % cell_blk(je, jb, 1)) + p_diag % vt(je, jk, jb) * (p_patch % edges % f_e(je, jb) + 0.5D0 * (zeta(jk, p_patch % edges % vertex_idx(je, jb, 1), p_patch % edges % vertex_blk(je, jb, 1)) + zeta(jk, p_patch % edges % vertex_idx(je, jb, 2), p_patch % edges % vertex_blk(je, jb, 2)))) + (p_int % c_lin_e(je, 1, jb) * z_w_con_c_full(p_patch % edges % cell_idx(je, jb, 1), jk, p_patch % edges % cell_blk(je, jb, 1)) + p_int % c_lin_e(je, 2, jb) * z_w_con_c_full(p_patch % edges % cell_idx(je, jb, 2), jk, p_patch % edges % cell_blk(je, jb, 2))) * (p_diag % vn_ie(je, jk, jb) - p_diag % vn_ie(je, jk + 1, jb)) / p_metrics % ddqz_z_full_e(je, jk, jb))
          END DO
        END DO
      ...
      ELSE
        DO je = i_startidx, i_endidx
          DO jk = 1, nlev
            p_diag % ddt_vn_apc_pc(je, jk, jb, ntnd) = - ((z_kin_hor_e(je, jk, jb) * (p_metrics % coeff_gradekin(je, 1, jb) - p_metrics % coeff_gradekin(je, 2, jb)) + p_metrics % coeff_gradekin(je, 2, jb) * z_ekinh(jk, p_patch % edges % cell_idx(je, jb, 2), p_patch % edges % cell_blk(je, jb, 2)) - p_metrics % coeff_gradekin(je, 1, jb) * z_ekinh(jk, p_patch % edges % cell_idx(je, jb, 1), p_patch % edges % cell_blk(je, jb, 1))) * p_metrics % deepatmo_gradh_mc(jk) + p_diag % vt(je, jk, jb) * (p_patch % edges % f_e(je, jb) + 0.5D0 * (zeta(jk, p_patch % edges % vertex_idx(je, jb, 1), p_patch % edges % vertex_blk(je, jb, 1)) + zeta(jk, p_patch % edges % vertex_idx(je, jb, 2), p_patch % edges % vertex_blk(je, jb, 2))) * p_metrics % deepatmo_gradh_mc(jk)) + (p_int % c_lin_e(je, 1, jb) * z_w_con_c_full(p_patch % edges % cell_idx(je, jb, 1), jk, p_patch % edges % cell_blk(je, jb, 1)) + p_int % c_lin_e(je, 2, jb) * z_w_con_c_full(p_patch % edges % cell_idx(je, jb, 2), jk, p_patch % edges % cell_blk(je, jb, 2))) * ((p_diag % vn_ie(je, jk, jb) - p_diag % vn_ie(je, jk + 1, jb)) / p_metrics % ddqz_z_full_e(je, jk, jb) + p_prog % vn(je, jk, jb) * p_metrics % deepatmo_invr_mc(jk) - p_patch % edges % ft_e(je, jb)))
          END DO
        END DO
        IF (p_diag % ddt_vn_adv_is_associated .OR. p_diag % ddt_vn_cor_is_associated) THEN
          DO jk = 1, nlev
