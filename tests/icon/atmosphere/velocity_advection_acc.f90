...
! the directive source tests/acc_residency_test.py reads.  GENERATED by
! tests/icon/atmosphere/annotate_velocity_acc.py -- edit that, not this.  TU variant: loopexch.
!
...
MODULE mo_lib_loopindices
  IMPLICIT NONE
  CONTAINS
  SUBROUTINE get_indices_c_lib(i_startidx_in, i_endidx_in, nproma, i_blk, i_startblk, i_endblk, i_startidx_out, &
    i_endidx_out)
  ...
  SUBROUTINE get_indices_e_lib(i_startidx_in, i_endidx_in, nproma, i_blk, i_startblk, i_endblk, i_startidx_out, &
    i_endidx_out)
    INTEGER, INTENT(IN) :: i_startidx_in
    INTEGER, INTENT(IN) :: i_endidx_in
    INTEGER, INTENT(IN) :: nproma
  ...
  SUBROUTINE get_indices_v_lib(i_startidx_in, i_endidx_in, nproma, i_blk, i_startblk, i_endblk, i_startidx_out, &
    i_endidx_out)
    INTEGER, INTENT(IN) :: i_startidx_in
    INTEGER, INTENT(IN) :: i_endidx_in
    INTEGER, INTENT(IN) :: nproma
...
MODULE mo_lib_divrot
  IMPLICIT NONE
  CONTAINS
  SUBROUTINE rot_vertex_ri_lib(vec_e, vert_edge_idx, vert_edge_blk, geofac_rot, rot_vec, i_startblk, i_endblk, &
    i_startidx_in, i_endidx_in, slev, elev, nproma, lacc, acc_async)
...
!$OMP DO PRIVATE(jb, jv, jk, i_startidx, i_endidx) SCHEDULE(guided)
    DO jb = i_startblk, i_endblk
      ...
      DO jv = i_startidx, i_endidx
        DO jk = slev, elev
          rot_vec(jk, jv, jb) = vec_e(vert_edge_idx(jv, jb, 1), jk, vert_edge_blk(jv, jb, 1)) * geofac_rot(jv, 1, &
            jb) + vec_e(vert_edge_idx(jv, jb, 2), jk, vert_edge_blk(jv, jb, 2)) * geofac_rot(jv, 2, jb) + &
            vec_e(vert_edge_idx(jv, jb, 3), jk, vert_edge_blk(jv, jb, 3)) * geofac_rot(jv, 3, jb) + &
            vec_e(vert_edge_idx(jv, jb, 4), jk, vert_edge_blk(jv, jb, 4)) * geofac_rot(jv, 4, jb) + &
            vec_e(vert_edge_idx(jv, jb, 5), jk, vert_edge_blk(jv, jb, 5)) * geofac_rot(jv, 5, jb) + &
            vec_e(vert_edge_idx(jv, jb, 6), jk, vert_edge_blk(jv, jb, 6)) * geofac_rot(jv, 6, jb)
        END DO
      END DO
      !$ACC END PARALLEL
    END DO
...
MODULE mo_lib_interpolation_scalar
  IMPLICIT NONE
  CONTAINS
  SUBROUTINE cells2verts_scalar_ri_lib(p_cell_in, vert_cell_idx, vert_cell_blk, coeff_int, p_vert_out, i_startblk, &
    i_endblk, i_startidx_in, i_endidx_in, slev, elev, nproma, lacc, acc_async)
    USE mo_fortran_tools, ONLY: set_acc_host_or_device
    USE mo_lib_loopindices, ONLY: get_indices_v_lib
...
!$OMP DO PRIVATE(jb, jv, jk, i_startidx, i_endidx) SCHEDULE(guided)
    DO jb = i_startblk, i_endblk
      ...
      DO jv = i_startidx, i_endidx
        DO jk = slev, elev
          p_vert_out(jk, jv, jb) = coeff_int(jv, 1, jb) * p_cell_in(vert_cell_idx(jv, jb, 1), jk, vert_cell_blk(jv, &
            jb, 1)) + coeff_int(jv, 2, jb) * p_cell_in(vert_cell_idx(jv, jb, 2), jk, vert_cell_blk(jv, jb, 2)) + &
            coeff_int(jv, 3, jb) * p_cell_in(vert_cell_idx(jv, jb, 3), jk, vert_cell_blk(jv, jb, 3)) + coeff_int(jv, &
            4, jb) * p_cell_in(vert_cell_idx(jv, jb, 4), jk, vert_cell_blk(jv, jb, 4)) + coeff_int(jv, 5, jb) * &
            p_cell_in(vert_cell_idx(jv, jb, 5), jk, vert_cell_blk(jv, jb, 5)) + coeff_int(jv, 6, jb) * &
            p_cell_in(vert_cell_idx(jv, jb, 6), jk, vert_cell_blk(jv, jb, 6))
        END DO
      END DO
      !$ACC END PARALLEL
    END DO
...
MODULE mo_nonhydro_types
  ...
  END TYPE t_nh_prog
  TYPE :: t_nh_diag
    REAL(KIND = 8), POINTER, CONTIGUOUS :: exner_pr(:, :, :), mass_fl_e(:, :, :), rho_ic(:, :, :), theta_v_ic(:, :, &
      :), grf_tend_vn(:, :, :), grf_tend_w(:, :, :), grf_tend_rho(:, :, :), grf_tend_mflx(:, :, :), grf_bdy_mflx(:, &
      :, :), grf_tend_thv(:, :, :), vn_ie_int(:, :, :), vn_ie_ubc(:, :, :), w_int(:, :, :), w_ubc(:, :, :), &
      theta_v_ic_int(:, :, :), theta_v_ic_ubc(:, :, :), rho_ic_int(:, :, :), rho_ic_ubc(:, :, :), mflx_ic_int(:, :, &
      :), mflx_ic_ubc(:, :, :)
    REAL(KIND = 8), POINTER, CONTIGUOUS :: vn_incr(:, :, :), exner_incr(:, :, :), rho_incr(:, :, :), vt(:, :, :), &
      ddt_exner_phy(:, :, :), ddt_vn_phy(:, :, :), exner_dyn_incr(:, :, :), vn_ie(:, :, :), w_concorr_c(:, :, :), &
      mass_fl_e_sv(:, :, :), ddt_vn_apc_pc(:, :, :, :), ddt_vn_cor_pc(:, :, :, :), ddt_w_adv_pc(:, :, :, :)
    REAL(KIND = 8), POINTER, CONTIGUOUS :: ddt_vn_dyn(:, :, :) => NULL(), ddt_vn_dmp(:, :, :) => NULL(), &
      ddt_vn_adv(:, :, :) => NULL(), ddt_vn_cor(:, :, :) => NULL(), ddt_vn_pgr(:, :, :) => NULL(), ddt_vn_phd(:, :, &
      :) => NULL(), ddt_vn_iau(:, :, :) => NULL(), ddt_vn_ray(:, :, :) => NULL(), ddt_vn_grf(:, :, :) => NULL()
    LOGICAL :: ddt_vn_dyn_is_associated = .FALSE., ddt_vn_dmp_is_associated = .FALSE., ddt_vn_adv_is_associated = &
      .FALSE., ddt_vn_cor_is_associated = .FALSE., ddt_vn_pgr_is_associated = .FALSE., ddt_vn_phd_is_associated = &
      .FALSE., ddt_vn_iau_is_associated = .FALSE., ddt_vn_ray_is_associated = .FALSE., ddt_vn_grf_is_associated = &
      .FALSE.
    REAL(KIND = 8) :: max_vcfl_dyn = 0.0D0
  END TYPE t_nh_diag
  TYPE :: t_nh_metrics
    REAL(KIND = 8), POINTER, CONTIGUOUS :: rayleigh_w(:), rayleigh_vn(:), scalfac_dd3d(:), hmask_dd3d(:, :), &
      vwind_expl_wgt(:, :), vwind_impl_wgt(:, :)
    REAL(KIND = 8), POINTER, CONTIGUOUS :: ddxn_z_full(:, :, :), ddxt_z_full(:, :, :), ddqz_z_full_e(:, :, :), &
      ddqz_z_half(:, :, :), inv_ddqz_z_full(:, :, :), wgtfac_c(:, :, :), wgtfac_e(:, :, :), wgtfacq_c(:, :, :), &
      wgtfacq_e(:, :, :), wgtfacq1_c(:, :, :), coeff_gradekin(:, :, :), coeff1_dwdz(:, :, :), coeff2_dwdz(:, :, :), &
      zdiff_gradp(:, :, :, :), coeff_gradp(:, :, :, :), exner_exfac(:, :, :), theta_ref_mc(:, :, :), theta_ref_me(:, &
      :, :), theta_ref_ic(:, :, :), exner_ref_mc(:, :, :), rho_ref_mc(:, :, :), rho_ref_me(:, :, :), &
      d_exner_dz_ref_ic(:, :, :), d2dexdz2_fac1_mc(:, :, :), d2dexdz2_fac2_mc(:, :, :), pg_exdist(:) => NULL()
    INTEGER, POINTER, CONTIGUOUS :: vertidx_gradp(:, :, :, :), pg_edgeidx(:), pg_edgeblk(:), pg_vertidx(:), &
      bdy_halo_c_idx(:), bdy_halo_c_blk(:), bdy_mflx_e_idx(:), bdy_mflx_e_blk(:) => NULL()
    REAL(KIND = 8), POINTER, CONTIGUOUS :: deepatmo_gradh_mc(:), deepatmo_divh_mc(:), deepatmo_invr_mc(:), &
      deepatmo_divzu_mc(:), deepatmo_divzl_mc(:), deepatmo_gradh_ifc(:), deepatmo_invr_ifc(:) => NULL()
    INTEGER :: pg_listdim
...
MODULE mo_math_divrot
  ...
  SUBROUTINE rot_vertex_ri(vec_e, ptr_patch, ptr_int, rot_vec, lacc, opt_slev, opt_elev, opt_rlend, opt_acc_async)
    ...
    CALL rot_vertex_ri_lib(vec_e, ptr_patch % verts % edge_idx, ptr_patch % verts % edge_blk, ptr_int % geofac_rot, &
      rot_vec, i_startblk, i_endblk, i_startidx_in, i_endidx_in, slev, elev, nproma, lacc = .TRUE., acc_async = &
      .TRUE.)
...
MODULE mo_icon_interpolation_scalar
  IMPLICIT NONE
  CONTAINS
  SUBROUTINE cells2verts_scalar_ri(p_cell_in, ptr_patch, c_int, p_vert_out, lacc, opt_slev, opt_elev, opt_rlstart, &
    ...
    CALL cells2verts_scalar_ri_lib(p_cell_in, ptr_patch % verts % cell_idx, ptr_patch % verts % cell_blk, c_int, &
      p_vert_out, i_startblk, i_endblk, i_startidx_in, i_endidx_in, slev, elev, nproma, lacc = .TRUE., acc_async = &
      .TRUE.)
...
MODULE mo_velocity_advection
  IMPLICIT NONE
  CONTAINS
  SUBROUTINE velocity_tendencies(p_prog, p_patch, p_int, p_metrics, p_diag, z_w_concorr_me, z_kin_hor_e, z_vt_ie, &
    ...
    END IF
    IF (.NOT. lvn_only) CALL cells2verts_scalar_ri(p_prog % w, p_patch, p_int % cells_aw_verts, z_w_v, lacc = &
      .TRUE., opt_rlend = (- 5), opt_acc_async = .TRUE.)
...
!$OMP DO PRIVATE(jb, jk, je, i_startidx, i_endidx) SCHEDULE(guided)
      DO jb = i_startblk, i_endblk
        ...
        DO je = i_startidx, i_endidx
          DO jk = 1, nlev
            p_diag % vt(je, jk, jb) = p_int % rbf_vec_coeff_e(1, je, jb) * p_prog % vn(p_patch % edges % &
              quad_idx(je, jb, 1), jk, p_patch % edges % quad_blk(je, jb, 1)) + p_int % rbf_vec_coeff_e(2, je, jb) * &
              p_prog % vn(p_patch % edges % quad_idx(je, jb, 2), jk, p_patch % edges % quad_blk(je, jb, 2)) + p_int &
              % rbf_vec_coeff_e(3, je, jb) * p_prog % vn(p_patch % edges % quad_idx(je, jb, 3), jk, p_patch % edges &
              % quad_blk(je, jb, 3)) + p_int % rbf_vec_coeff_e(4, je, jb) * p_prog % vn(p_patch % edges % &
              quad_idx(je, jb, 4), jk, p_patch % edges % quad_blk(je, jb, 4))
          END DO
        END DO
        !$ACC END PARALLEL
        !$ACC PARALLEL DEFAULT(PRESENT) ASYNC(1)
        ...
        DO jk = 2, nlev
          DO je = i_startidx, i_endidx
            p_diag % vn_ie(je, jk, jb) = p_metrics % wgtfac_e(je, jk, jb) * p_prog % vn(je, jk, jb) + (1.0D0 - &
              p_metrics % wgtfac_e(je, jk, jb)) * p_prog % vn(je, jk - 1, jb)
        ...
        IF (.NOT. lvn_only) THEN
          ...
          DO jk = 2, nlev
            DO je = i_startidx, i_endidx
              z_vt_ie(je, jk, jb) = p_metrics % wgtfac_e(je, jk, jb) * p_diag % vt(je, jk, jb) + (1.0D0 - p_metrics &
                % wgtfac_e(je, jk, jb)) * p_diag % vt(je, jk - 1, jb)
            END DO
          END DO
          !$ACC END PARALLEL
        END IF
        ...
        DO jk = nflatlev_jg, nlev
          DO je = i_startidx, i_endidx
            z_w_concorr_me(je, jk, jb) = p_prog % vn(je, jk, jb) * p_metrics % ddxn_z_full(je, jk, jb) + p_diag % &
              vt(je, jk, jb) * p_metrics % ddxt_z_full(je, jk, jb)
          END DO
        END DO
        !$ACC END PARALLEL
        IF (.NOT. l_vert_nested) THEN
          ...
          DO je = i_startidx, i_endidx
            ...
            p_diag % vn_ie(je, nlevp1, jb) = p_metrics % wgtfacq_e(je, 1, jb) * p_prog % vn(je, nlev, jb) + &
              p_metrics % wgtfacq_e(je, 2, jb) * p_prog % vn(je, nlev - 1, jb) + p_metrics % wgtfacq_e(je, 3, jb) * &
              p_prog % vn(je, nlev - 2, jb)
          END DO
          !$ACC END PARALLEL
        ELSE
          !$ACC PARALLEL DEFAULT(PRESENT) ASYNC(1)
        ...
        DO je = i_startidx, i_endidx
          DO jk = 1, nlev
            z_v_grad_w(jk, je, jb) = p_diag % vn_ie(je, jk, jb) * p_patch % edges % inv_dual_edge_length(je, jb) * &
              (p_prog % w(p_patch % edges % cell_idx(je, jb, 1), jk, p_patch % edges % cell_blk(je, jb, 1)) - p_prog &
              % w(p_patch % edges % cell_idx(je, jb, 2), jk, p_patch % edges % cell_blk(je, jb, 2))) + z_vt_ie(je, &
              jk, jb) * p_patch % edges % inv_primal_edge_length(je, jb) * p_patch % edges % tangent_orientation(je, &
              jb) * (z_w_v(jk, p_patch % edges % vertex_idx(je, jb, 1), p_patch % edges % vertex_blk(je, jb, 1)) - &
              z_w_v(jk, p_patch % edges % vertex_idx(je, jb, 2), p_patch % edges % vertex_blk(je, jb, 2)))
          END DO
        END DO
        !$ACC END PARALLEL
      END DO
...
!$OMP DO PRIVATE(jb, jk, je, i_startidx, i_endidx) SCHEDULE(guided)
      DO jb = i_startblk, i_endblk
        ...
        DO je = i_startidx, i_endidx
          DO jk = 1, nlev
            z_v_grad_w(jk, je, jb) = z_v_grad_w(jk, je, jb) * p_metrics % deepatmo_gradh_ifc(jk) + p_diag % &
              vn_ie(je, jk, jb) * (p_diag % vn_ie(je, jk, jb) * p_metrics % deepatmo_invr_ifc(jk) - p_patch % edges &
              % ft_e(je, jb)) + z_vt_ie(je, jk, jb) * (z_vt_ie(je, jk, jb) * p_metrics % deepatmo_invr_ifc(jk) + &
...
!$OMP            z_w_concorr_mc, difcoef, vcfl, maxvcfl, cfl_clipping, clip_count) SCHEDULE(guided)
    DO jb = i_startblk, i_endblk
      ...
      DO jc = i_startidx, i_endidx
        DO jk = 1, nlev
          z_ekinh(jk, jc, jb) = p_int % e_bln_c_s(jc, 1, jb) * z_kin_hor_e(p_patch % cells % edge_idx(jc, jb, 1), &
            jk, p_patch % cells % edge_blk(jc, jb, 1)) + p_int % e_bln_c_s(jc, 2, jb) * z_kin_hor_e(p_patch % cells &
            % edge_idx(jc, jb, 2), jk, p_patch % cells % edge_blk(jc, jb, 2)) + p_int % e_bln_c_s(jc, 3, jb) * &
            z_kin_hor_e(p_patch % cells % edge_idx(jc, jb, 3), jk, p_patch % cells % edge_blk(jc, jb, 3))
        END DO
      END DO
      !$ACC END PARALLEL
      IF (istep == 1) THEN
        ...
        DO jc = i_startidx, i_endidx
          DO jk = nflatlev_jg, nlev
            z_w_concorr_mc(jc, jk) = p_int % e_bln_c_s(jc, 1, jb) * z_w_concorr_me(p_patch % cells % edge_idx(jc, &
              jb, 1), jk, p_patch % cells % edge_blk(jc, jb, 1)) + p_int % e_bln_c_s(jc, 2, jb) * &
              z_w_concorr_me(p_patch % cells % edge_idx(jc, jb, 2), jk, p_patch % cells % edge_blk(jc, jb, 2)) + &
              p_int % e_bln_c_s(jc, 3, jb) * z_w_concorr_me(p_patch % cells % edge_idx(jc, jb, 3), jk, p_patch % &
              cells % edge_blk(jc, jb, 3))
          END DO
        END DO
        !$ACC END PARALLEL
        !$ACC PARALLEL DEFAULT(PRESENT) ASYNC(1)
        ...
        DO jk = nflatlev_jg + 1, nlev
          DO jc = i_startidx, i_endidx
            p_diag % w_concorr_c(jc, jk, jb) = p_metrics % wgtfac_c(jc, jk, jb) * z_w_concorr_mc(jc, jk) + (1.0D0 - &
              p_metrics % wgtfac_c(jc, jk, jb)) * z_w_concorr_mc(jc, jk - 1)
          END DO
        END DO
        !$ACC END PARALLEL
      END IF
      ...
      DO jk = MAX(3, nrdmax_jg - 2), nlev - 3
        clip_count = 0
        !$ACC LOOP VECTOR REDUCTION(+: clip_count)
        DO jc = i_startidx, i_endidx
      ...
      DO jk = 2, nlev
        DO jc = i_startidx_2, i_endidx_2
          p_diag % ddt_w_adv_pc(jc, jk, jb, ntnd) = - z_w_con_c(jc, jk) * (p_prog % w(jc, jk - 1, jb) * p_metrics % &
            coeff1_dwdz(jc, jk, jb) - p_prog % w(jc, jk + 1, jb) * p_metrics % coeff2_dwdz(jc, jk, jb) + p_prog % &
            w(jc, jk, jb) * (p_metrics % coeff2_dwdz(jc, jk, jb) - p_metrics % coeff1_dwdz(jc, jk, jb)))
        END DO
      END DO
      !$ACC LOOP GANG(STATIC: 1) VECTOR TILE(32, 4)
      DO jc = i_startidx_2, i_endidx_2
        DO jk = 2, nlev
          p_diag % ddt_w_adv_pc(jc, jk, jb, ntnd) = p_diag % ddt_w_adv_pc(jc, jk, jb, ntnd) + p_int % e_bln_c_s(jc, &
            1, jb) * z_v_grad_w(jk, p_patch % cells % edge_idx(jc, jb, 1), p_patch % cells % edge_blk(jc, jb, 1)) + &
            p_int % e_bln_c_s(jc, 2, jb) * z_v_grad_w(jk, p_patch % cells % edge_idx(jc, jb, 2), p_patch % cells % &
            edge_blk(jc, jb, 2)) + p_int % e_bln_c_s(jc, 3, jb) * z_v_grad_w(jk, p_patch % cells % edge_idx(jc, jb, &
            3), p_patch % cells % edge_blk(jc, jb, 3))
        END DO
      END DO
      !$ACC END PARALLEL
      IF (lextra_diffu) THEN
          ...
          IF (levmask(jb, jk)) THEN
            ...
            DO jc = i_startidx_2, i_endidx_2
              IF (cfl_clipping(jc, jk) .AND. p_patch % cells % decomp_info % owner_mask(jc, jb)) THEN
                difcoef = scalfac_exdiff * MIN(0.85D0 - cfl_w_limit * dtime, ABS(z_w_con_c(jc, jk)) * dtime / &
                  p_metrics % ddqz_z_half(jc, jk, jb) - cfl_w_limit * dtime)
                p_diag % ddt_w_adv_pc(jc, jk, jb, ntnd) = p_diag % ddt_w_adv_pc(jc, jk, jb, ntnd) + difcoef * &
                  p_patch % cells % area(jc, jb) * (p_prog % w(jc, jk, jb) * p_int % geofac_n2s(jc, 1, jb) + p_prog &
                  % w(p_patch % cells % neighbor_idx(jc, jb, 1), jk, p_patch % cells % neighbor_blk(jc, jb, 1)) * &
                  p_int % geofac_n2s(jc, 2, jb) + p_prog % w(p_patch % cells % neighbor_idx(jc, jb, 2), jk, p_patch &
                  % cells % neighbor_blk(jc, jb, 2)) * p_int % geofac_n2s(jc, 3, jb) + p_prog % w(p_patch % cells % &
                  neighbor_idx(jc, jb, 3), jk, p_patch % cells % neighbor_blk(jc, jb, 3)) * p_int % geofac_n2s(jc, &
                  4, jb))
              END IF
            END DO
          END IF
...
!$OMP DO PRIVATE(jb, jk, je, i_startidx, i_endidx, ie, w_con_e, difcoef) SCHEDULE(guided)
    DO jb = i_startblk, i_endblk
      ...
      IF (.NOT. ldeepatmo) THEN
        ...
        DO je = i_startidx, i_endidx
          DO jk = 1, nlev
            p_diag % ddt_vn_apc_pc(je, jk, jb, ntnd) = - (z_kin_hor_e(je, jk, jb) * (p_metrics % coeff_gradekin(je, &
              1, jb) - p_metrics % coeff_gradekin(je, 2, jb)) + p_metrics % coeff_gradekin(je, 2, jb) * z_ekinh(jk, &
              p_patch % edges % cell_idx(je, jb, 2), p_patch % edges % cell_blk(je, jb, 2)) - p_metrics % &
              coeff_gradekin(je, 1, jb) * z_ekinh(jk, p_patch % edges % cell_idx(je, jb, 1), p_patch % edges % &
              cell_blk(je, jb, 1)) + p_diag % vt(je, jk, jb) * (p_patch % edges % f_e(je, jb) + 0.5D0 * (zeta(jk, &
              p_patch % edges % vertex_idx(je, jb, 1), p_patch % edges % vertex_blk(je, jb, 1)) + zeta(jk, p_patch % &
              edges % vertex_idx(je, jb, 2), p_patch % edges % vertex_blk(je, jb, 2)))) + (p_int % c_lin_e(je, 1, &
              jb) * z_w_con_c_full(p_patch % edges % cell_idx(je, jb, 1), jk, p_patch % edges % cell_blk(je, jb, 1)) &
              + p_int % c_lin_e(je, 2, jb) * z_w_con_c_full(p_patch % edges % cell_idx(je, jb, 2), jk, p_patch % &
              edges % cell_blk(je, jb, 2))) * (p_diag % vn_ie(je, jk, jb) - p_diag % vn_ie(je, jk + 1, jb)) / &
      ...
      ELSE
        ...
        DO je = i_startidx, i_endidx
          DO jk = 1, nlev
            p_diag % ddt_vn_apc_pc(je, jk, jb, ntnd) = - ((z_kin_hor_e(je, jk, jb) * (p_metrics % coeff_gradekin(je, &
              ...
              p_patch % edges % cell_idx(je, jb, 2), p_patch % edges % cell_blk(je, jb, 2)) - p_metrics % &
              coeff_gradekin(je, 1, jb) * z_ekinh(jk, p_patch % edges % cell_idx(je, jb, 1), p_patch % edges % &
              cell_blk(je, jb, 1))) * p_metrics % deepatmo_gradh_mc(jk) + p_diag % vt(je, jk, jb) * (p_patch % edges &
              % f_e(je, jb) + 0.5D0 * (zeta(jk, p_patch % edges % vertex_idx(je, jb, 1), p_patch % edges % &
              vertex_blk(je, jb, 1)) + zeta(jk, p_patch % edges % vertex_idx(je, jb, 2), p_patch % edges % &
              vertex_blk(je, jb, 2))) * p_metrics % deepatmo_gradh_mc(jk)) + (p_int % c_lin_e(je, 1, jb) * &
              z_w_con_c_full(p_patch % edges % cell_idx(je, jb, 1), jk, p_patch % edges % cell_blk(je, jb, 1)) + &
              p_int % c_lin_e(je, 2, jb) * z_w_con_c_full(p_patch % edges % cell_idx(je, jb, 2), jk, p_patch % edges &
              % cell_blk(je, jb, 2))) * ((p_diag % vn_ie(je, jk, jb) - p_diag % vn_ie(je, jk + 1, jb)) / p_metrics % &
              ddqz_z_full_e(je, jk, jb) + p_prog % vn(je, jk, jb) * p_metrics % deepatmo_invr_mc(jk) - p_patch % &
              edges % ft_e(je, jb)))
          END DO
        END DO
        !$ACC END PARALLEL
        IF (p_diag % ddt_vn_adv_is_associated .OR. p_diag % ddt_vn_cor_is_associated) THEN
          ...
          DO jk = 1, nlev
            DO je = i_startidx, i_endidx
              p_diag % ddt_vn_cor_pc(je, jk, jb, ntnd) = - (+ p_diag % vt(je, jk, jb) * (p_patch % edges % f_e(je, &
                jb)) + (p_int % c_lin_e(je, 1, jb) * z_w_con_c_full(p_patch % edges % cell_idx(je, jb, 1), jk, &
                p_patch % edges % cell_blk(je, jb, 1)) + p_int % c_lin_e(je, 2, jb) * z_w_con_c_full(p_patch % edges &
                % cell_idx(je, jb, 2), jk, p_patch % edges % cell_blk(je, jb, 2))) * (- p_patch % edges % ft_e(je, &
                jb)))
            END DO
          END DO
          !$ACC END PARALLEL
      ...
      IF (lextra_diffu) THEN
        ...
        DO jk = MAX(3, nrdmax_jg - 2), nlev - 4
          IF (levelmask(jk) .OR. levelmask(jk + 1)) THEN
            ...
            DO je = i_startidx, i_endidx
              w_con_e = p_int % c_lin_e(je, 1, jb) * z_w_con_c_full(p_patch % edges % cell_idx(je, jb, 1), jk, &
                p_patch % edges % cell_blk(je, jb, 1)) + p_int % c_lin_e(je, 2, jb) * z_w_con_c_full(p_patch % edges &
                % cell_idx(je, jb, 2), jk, p_patch % edges % cell_blk(je, jb, 2))
              IF (ABS(w_con_e) > cfl_w_limit * p_metrics % ddqz_z_full_e(je, jk, jb)) THEN
                difcoef = scalfac_exdiff * MIN(0.85D0 - cfl_w_limit * dtime, ABS(w_con_e) * dtime / p_metrics % &
                  ddqz_z_full_e(je, jk, jb) - cfl_w_limit * dtime)
                p_diag % ddt_vn_apc_pc(je, jk, jb, ntnd) = p_diag % ddt_vn_apc_pc(je, jk, jb, ntnd) + difcoef * &
                  p_patch % edges % area_edge(je, jb) * (p_int % geofac_grdiv(je, 1, jb) * p_prog % vn(je, jk, jb) + &
                  p_int % geofac_grdiv(je, 2, jb) * p_prog % vn(p_patch % edges % quad_idx(je, jb, 1), jk, p_patch % &
                  edges % quad_blk(je, jb, 1)) + p_int % geofac_grdiv(je, 3, jb) * p_prog % vn(p_patch % edges % &
                  quad_idx(je, jb, 2), jk, p_patch % edges % quad_blk(je, jb, 2)) + p_int % geofac_grdiv(je, 4, jb) &
                  * p_prog % vn(p_patch % edges % quad_idx(je, jb, 3), jk, p_patch % edges % quad_blk(je, jb, 3)) + &
                  p_int % geofac_grdiv(je, 5, jb) * p_prog % vn(p_patch % edges % quad_idx(je, jb, 4), jk, p_patch % &
                  edges % quad_blk(je, jb, 4)) + p_patch % edges % tangent_orientation(je, jb) * p_patch % edges % &
                  inv_primal_edge_length(je, jb) * (zeta(jk, p_patch % edges % vertex_idx(je, jb, 2), p_patch % &
                  edges % vertex_blk(je, jb, 2)) - zeta(jk, p_patch % edges % vertex_idx(je, jb, 1), p_patch % edges &
                  % vertex_blk(je, jb, 1))))
              END IF
            END DO
          END IF
