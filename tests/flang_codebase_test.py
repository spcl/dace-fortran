...
@pytest.mark.skipif(not _HAVE_OPENMPI,
                    reason="OpenMPI include not on this system")
...
@pytest.mark.skipif(not _HAVE_ICON,
                    reason="ICON checkout not present at the standard path")
def test_extract_make_compile_args_for_icon_velocity():
    ...
    args = extract_make_compile_args(
        makefile_dir=_ICON_BUILD,
...
def test_prepare_translation_unit_flang_clean_on_icon_velocity(tmp_path: Path):
    ...
    result = subprocess.run(
        [
            "flang-new-21", "-fc1", "-cpp", "-U_OPENMP", "-U_OPENACC",
            *flang_flags,
            "-emit-hlfir", str(tu_path), "-o", str(hlfir_path),
        ],
        capture_output=True, text=True,
    )
    err_lines = [ln for ln in result.stderr.splitlines() if "error:" in ln]
    assert not err_lines, (
        f"flang reported {len(err_lines)} error(s); "
        f"first 5:\n" + "\n".join(err_lines[:5]))
